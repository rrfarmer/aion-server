...
|---|---|---|---|
| Static bonus stat entries | `ItemInfoBlob.addBonusBlobEntry` + `BonusInfoBlobEntry` writes id/value/rate flag for bonus modifiers without conditions. | `ItemTemplateSummary.StatModifiers` exists, but `WriteItemInfoBlob` never emits `STAT_BONUSES` entry id `0x0A`. | Any item template with eligible bonus modifiers will have missing blob entries and different blob size/order. |
| Fusion random bonus id | `CompositeItemBlobEntry` writes `ownerItem.getFusionedItemBonusStatsId()`. | `InventoryItem.FusionRandomBonus` exists, but `WriteCompositeItemBlob` writes trailing zero after optional fusion socket. | Fused items with bonus stats cannot match Java bytes. |
