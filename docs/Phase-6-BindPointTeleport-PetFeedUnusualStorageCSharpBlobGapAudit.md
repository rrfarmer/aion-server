...
| Cleanup/seal warehouse restriction flag | `GeneralInfoBlobEntry` calls `DataManager.ITEM_CLEAN_UP.hasAccountOrLegionWhStorabilityDisabled(...) ? 3 : 0`. | UOW-1393 added `StaticData.ItemRestrictionCleanups`, but no cleanup table input reaches `WriteItemInfoBlob`; it still writes zero. | Items restricted from account/legion warehouse storability still differ until packet call sites pass an explicit cleanup/seal flag. |
