...
## Update After UOW-1368

A read-only output-path audit now defines safety rules for the future unusual-storage JSON writer: validate and normalize the configured output directory, keep output under `./parity-artifacts/pet-feed-unusual-storage/java` by default, sanitize deterministic filename fragments, use temporary sibling files plus atomic move when supported, and avoid filesystem work in the packet serialization callback. The current snapshot likely needs player id or a sequence number before collision-resistant filenames can be generated. Live bind-point fanout remains blocked because no output helper, JSON serialization, file output, raw/canonical bytes, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, or live unusual-storage hydration/mutation/dispatch/runtime comparison exists.
...
## Update After UOW-1373

A read-only encode-time field audit now maps most schema item/template placeholders to Java observer-time sources available through `SM_WAREHOUSE_ADD_ITEM.getFirstItem()`: item id, count, item location, equipment slot, template id, localized name, pack count, expire time, temporary exchange times, charge, enchant level, item mask, and color. Remaining gaps are storage type name, captured epoch seconds, item blob hex, and packet body/canonical hex. Live bind-point fanout remains blocked because no encode-time item snapshot shell, Java compile validation, JSON serialization, file output, raw/canonical bytes, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, or live unusual-storage hydration/mutation/dispatch/runtime comparison exists.

## Update After UOW-1374

`PetFeedUnusualStorageArtifactCapture` now captures a passive encode-time item snapshot from `SM_WAREHOUSE_ADD_ITEM.getFirstItem()` during packet observation and feeds schema item fields from that snapshot. The shell records item id/count/location/equipment slot/template id/localized name/pack count/expire time/temporary exchange absolute and remaining values/charge/enchant/mask/color, but still emits no JSON and retains no raw/canonical packet bytes. Live bind-point fanout remains blocked because Java compile validation, runtime artifact validation, fastjson2 serialization, file output, raw/canonical bytes, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1375

A read-only byte retention audit now confirms future packet bytes should be copied from the observer's read-only clear-frame duplicate after length stamping and before encryption, using absolute indexed reads so capture never mutates buffer position or packet contents. Future fields should distinguish full clear-frame hex, body hex from offset `7`, and canonical payload hex, with compact uppercase JSON-friendly hex instead of `NetworkUtils.toHex` formatted log dumps. Live bind-point fanout remains blocked because no byte retention fields, compact hex helper, JSON serialization, file output, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, or live unusual-storage hydration/mutation/dispatch/runtime comparison exists.

## Update After UOW-1376

`PetFeedUnusualStorageArtifactCapture.PacketSnapshot` now passively retains compact uppercase `clearFrameHex`, `bodyHex` from offset `7`, and `canonicalPayloadHex` equal to body hex, copied with absolute reads from the observer's read-only clear-frame duplicate. The schema packet DTO now emits body/canonical hex, but no JSON/file output exists. Live bind-point fanout remains blocked because Java compile validation, runtime artifact validation, item blob hex retention, fastjson2 serialization, file output, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1377

A read-only item-blob hex retention audit now documents two future byte-retention options: authoritative packet-body slicing through `SM_WAREHOUSE_ADD_ITEM` and simpler observer-time dedicated `ItemInfoBlob` buffer serialization. The recommended next step is dedicated-buffer `itemBlob.hex` retention with an explicit warning that it is observer-time reserialization until a later packet-slice verifier compares it against `bodyHex`. Live bind-point fanout remains blocked because Java compile validation, runtime artifact validation, item blob hex implementation, packet-slice verification, fastjson2 serialization, file output, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1378

`PetFeedUnusualStorageArtifactCapture.ItemBlobSnapshot` now passively retains compact uppercase `itemBlob.hex` by serializing the observed Java `ItemInfoBlob` into a dedicated buffer sized as `2 + blob.size()`. The schema DTO now emits that hex and documents it as observer-time reserialization until a future packet-body slice verifier confirms it against the actual warehouse-add body bytes. Live bind-point fanout remains blocked because Java compile validation, runtime artifact validation, packet-slice verification, fastjson2 serialization, file output, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1379

A read-only packet-body blob-slice audit now maps exact `SM_WAREHOUSE_ADD_ITEM.bodyHex` offsets for the future verifier: 14 fixed bytes after warehouse route/object/template/marker fields, then Java UTF-16 localized-name bytes including the null terminator, then the two-byte `ItemInfoBlob` size prefix and payload. The future verifier should compare that authoritative packet-body slice against observer-time `itemBlob.hex` and fail closed when offsets or names cannot be derived. Live bind-point fanout remains blocked because Java compile validation, runtime artifact validation, packet-slice verifier implementation, fastjson2 serialization, file output, C# artifact reader/schema validation, warehouse-add byte comparison, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.
...
## Update After UOW-1388

The C# item-blob writer now serializes Java-shaped `STAT_BONUSES` entries after `PREMIUM_OPTION` and before `GENERAL_INFO` for condition-free bonus template modifiers with known nonzero Java stat masks. Focused regression coverage verifies `MAXHP`, `ATTACK_SPEED` sign inversion/rate flag, non-bonus skip, and conditioned skip, and unusual-storage diagnostics no longer list `STAT_BONUSES` as a known serializer gap. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, temporary-exchange/seal fields, plume tempering stats, conditioning presence, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1389

A read-only general-info input audit now confirms Java temporary-exchange serialization depends on per-item absolute `temporaryExchangeTime`, while the cleanup/seal flag depends on `DataManager.ITEM_CLEAN_UP.hasAccountOrLegionWhStorabilityDisabled(itemId)` rather than only the already-mutated item mask. C# currently lacks both an inventory-item temporary-exchange epoch field and an item cleanup dataholder. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, temporary-exchange model hydration, cleanup/seal static data ownership, plume tempering stats, conditioning presence, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1390

A read-only plume tempering stat payload audit now confirms C# already has the inputs needed to serialize Java plume stat pairs in `EnchantInfoBlobEntry`: item group plume status, `TemperingName`, item tempering level, and random plume bonus. Java packet constants are HP `(42, 150)`, physical attack `(30, 4)`, and magical boost `(35, 20)`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, plume stat-pair serializer implementation/tests, temporary-exchange model hydration, cleanup/seal static data ownership, conditioning presence, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1391

The C# full item-blob enchant-info writer now serializes Java-shaped plume tempering stat pairs when template context identifies a tempered plume: HP `(42, 150 * tempering)` plus physical `(30, 4 * tempering + randomPlumeBonus)` for `TSHIRT_PHYSICAL`, or magical `(35, 20 * tempering + randomPlumeBonus)` otherwise. Focused regression coverage verifies physical plume, magical plume, and non-plume zero behavior, and unusual-storage diagnostics no longer list plume tempering stats as a known serializer gap. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, temporary-exchange model hydration, cleanup/seal static data ownership, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1392

A read-only cleanup/seal static-data ownership audit now locates Java `item_restriction_cleanups` under `game-server/data/static_data/items`, confirms `StaticData.itemCleanup` and `DataManager.ITEM_CLEAN_UP` ownership, and documents the exact general-info predicate `itemId match && (awh == 0 || lwh == 0)`. C# still lacks the dataholder/parser and packet flag plumbing. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, cleanup/seal C# static-data projection, temporary-exchange model hydration, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1393

The C# static-data loader now projects Java `item_restriction_cleanups` into `StaticData.ItemRestrictionCleanups`, preserving missing byte attributes as `-1` and exposing the Java packet predicate `awh == 0 || lwh == 0`. Focused tests cover synthetic cleanup rows and the real Java static-data row for item id `188053996`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, cleanup/seal packet flag plumbing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1394

The C# item-blob serializer now accepts an explicit cleanup/seal general-info flag and writes Java's `3/0` value into the `GENERAL_INFO` `H` field. Enter-world inventory and warehouse login packet creation now pass `StaticData.ItemRestrictionCleanups`, and focused packet coverage verifies restricted item id `188053996` writes flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, service-produced cleanup/seal flag plumbing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1395

`SmWarehouseAddItem` now accepts an explicit cleanup/seal general-info flag and passes it into the shared item blob writer. Focused packet coverage verifies an account-warehouse add packet can carry restricted item id `188053996` with nested `GENERAL_INFO` cleanup/seal flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, pet-feed metadata cleanup/seal flag context, inventory add/update and mail cleanup flag plumbing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1396

Pet-feed unlock packet metadata now carries an explicit precomputed cleanup/seal general-info flag through `PetFeedUnlockPacketContext` and into regular, account, legion item, and guarded unusual-storage `SmWarehouseAddItem` construction. Focused bridge tests verify restricted item id `188053996` writes nested `GENERAL_INFO` cleanup/seal flag `3` for those metadata paths. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, live pet-feed context flag sourcing, inventory add/update and mail cleanup flag plumbing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1397

`SmInventoryAddItem` and normal full-blob `SmInventoryUpdateItem` now accept explicit cleanup/seal general-info flag inputs and pass them into the shared item blob writer. Focused packet coverage verifies restricted item id `188053996` writes nested `GENERAL_INFO` cleanup/seal flag `3` in inventory add and update packets. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, live inventory/pet-feed context flag sourcing, mail cleanup flag plumbing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1398

`SmMailService` read-letter attached item serialization now accepts an explicit cleanup/seal general-info flag and passes it into the shared item blob writer. Focused packet coverage verifies restricted item id `188053996` writes nested `GENERAL_INFO` cleanup/seal flag `3` for a mail attached item. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, live inventory/pet-feed/mail context flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1399

The live `CmReadMail` caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` through `GameServerConnection.GetGeneralInfoWarehouseRestrictionFlag` and passes it into `SmMailService.CreateReadPacket`. Focused helper coverage verifies the Java cleanup predicate and default-zero fallback. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, live broker/inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1400

The live broker-buy caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds `SmInventoryAddItem.CreateBrokerBuy` for bought items. Focused packet coverage verifies the broker-buy `SM_INVENTORY_ADD_ITEM` path can carry restricted item id `188053996` with nested `GENERAL_INFO` flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1401

The live broker cancel-return caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds `SmInventoryAddItem.CreateBrokerReturn` for returned registered items. Focused packet coverage verifies the broker-return `SM_INVENTORY_ADD_ITEM` path can carry restricted item id `188053996` with nested `GENERAL_INFO` flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, broker settlement and remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1402

The live broker settlement returned-item caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds `SmInventoryAddItem.CreateItemCollect` for returned non-sold settlement items. Settlement kinah updates remain unchanged because they do not require full item-blob cleanup/seal context. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1403

The live composition reward caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds both item-collect adds and full-blob inventory updates for composition reward items. Parallel read-only audits identified house-use rewards and world loot as follow-up cleanup/seal caller families; world loot needs the cleanup table passed into `WorldNpcLootService` before packet construction. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1404

The live house-use reward caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds both item-collect adds and full-blob inventory updates for house-object reward items. Parallel read-only audits identified `SendAssemblyRewardPacketsAsync` as the next smallest adjacent helper, while no existing connection-level house-use reward socket test covers add/update reward emissions. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1405

The live assembly reward caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds both item-collect adds and full-blob inventory updates for assembly reward items. Focused connection-level tests now cover restricted assembly reward add and stack-merge packets carrying nested `GENERAL_INFO` flag `3`. Parallel read-only audit identified `SendExpExtractRewardPacketsAsync` as the next smallest adjacent helper. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1406

The live XP extraction reward caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds both item-collect adds and full-blob inventory updates for XP extraction reward items. Focused connection-level tests cover restricted XP extraction reward add and stack-merge packets carrying nested `GENERAL_INFO` flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1407

The live extraction reward caller now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` and feeds both item-collect adds and full-blob inventory updates for extracted enchant-stone reward items. Focused connection-level tests cover restricted extraction reward add and stack-merge packets carrying nested `GENERAL_INFO` flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1408

The live decompose reward sender now computes Java's cleanup/seal general-info flag from `StaticData.ItemRestrictionCleanups` for normal and selectable decompose reward packets, covering both decomposable add packets and full-blob inventory updates. Focused normal-decompose connection tests cover restricted reward add and stack-merge packets carrying nested `GENERAL_INFO` flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.

## Update After UOW-1409

The live world-loot item collection caller now passes `StaticData.ItemRestrictionCleanups` into `WorldNpcLootService.RequestDropItem`, and the service computes Java's cleanup/seal general-info flag for item-collect add packets and full-blob inventory updates. Focused world-loot service tests cover restricted solo-loot add and stack-merge packets carrying nested `GENERAL_INFO` flag `3`. Live bind-point fanout remains blocked because Java compile/runtime validation, runtime artifact generation, warehouse-add byte comparison, quest/custom reward cleanup flag sourcing, remaining live inventory/pet-feed cleanup flag sourcing, temporary-exchange model/template fields, runtime conditioning presence, time-normalized expiration/dye comparison, broker plume template context, runtime lifecycle validation, and live unusual-storage hydration/mutation/dispatch/runtime comparison are still missing.
