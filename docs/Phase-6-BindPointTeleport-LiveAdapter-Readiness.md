...
| Inventory mutation | `tryDecreaseKinah(price, DEC_KINAH_FLY)` with persistence and packet order | Planner only; callback bridge consumes supplied success/failure metadata; UOW-1222 audit completed | First add named `SmInventoryUpdateItem.DecreaseKinahFly` packet coverage, then isolate live inventory mutation/persistence. |
