...
| Inventory mutation | `tryDecreaseKinah(price, DEC_KINAH_FLY)` with persistence and packet order | Planner only; callback bridge consumes supplied success/failure metadata | Defer live mutation until update packet and persistence path are isolated. |
