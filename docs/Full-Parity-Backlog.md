...
- `SM_QUEST_COMPLETED_LIST` — updateMode + `List<QuestState>`. **[NOT done — earlier "DONE batch 10" claim was wrong (no fixture/case).]** Demoted to **T1-seam (DataManager-holder)**: `qs.canRepeat()` reads `DataManager.QUEST_DATA.getQuestById(questId)` per entry -> needs QUEST_DATA holder loaded + real questIds, OR pin via the 7-arg ctor with a loaded questId.
