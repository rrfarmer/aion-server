...
| `com.aionemu.gameserver.network.aion.serverpackets.SM_QUEST_ACTION`; `QuestEngine.onQuestCompleted`; `PlayerQuestListDAO.store`; `PlayerNpcFactionsDAO.storeNpcFactions` | `Aion.GameServer.Services.QuestPersistencePlanService`; `Aion.GameServer.Services.NpcFactionPersistencePlanService`; `Aion.GameServer.Services.QuestFinishOperationPlanService` | Packet / Callback / Repository Dependency | Partial | Unit Tested for quest and NPC-faction persistence planning and composition | Needs Verification | UOW-1024 composes non-live quest/NPC-faction persistence plans into quest-finish operation planning after nearby refresh. C# still lacks live callback runtime, DAO writes, Java persistent-state tracking in snapshots, full store-player ordering, and runtime comparison. |
| `com.aionemu.gameserver.questEngine.QuestEngine.onQuestCompleted`; `AbstractQuestHandler.onQuestCompletedEvent`; `QuestEnv` | `Aion.GameServer.Services.QuestCompletionCallbackPlanService`; `Aion.GameServer.Services.QuestFinishOperationPlanService` | Callback Dependency | Partial | Unit Tested | Needs Verification | UOW-1026 composes non-live callback dispatch descriptors into quest-finish operation planning after the quest update packet and before NPC-faction completion. C# still lacks live handler execution, callback-triggered quest packets, mutable `QuestEnv`, callback result modeling, and runtime comparison. |
| `com.aionemu.gameserver.questEngine.handlers.AbstractQuestHandler.defaultOnQuestCompletedEvent`; `QuestService.addOrUpdateQuest`; `SM_QUEST_ACTION` | `Aion.GameServer.Services.QuestCompletionFollowUpPlanService`; `Aion.GameServer.Services.QuestCompletionCallbackPlanService`; `Aion.GameServer.Services.QuestFinishOperationPlanService` | Callback Follow-Up Dependency | Partial | Unit Tested | Needs Verification | UOW-1029 adds quest-finish regression coverage for nested non-live follow-up result payloads after UOW-1028 composed them into callback descriptors. C# still lacks Java start-condition execution, XML condition recursion, live quest-state mutation, packet sends, and runtime comparison. |
...
- Quest-finish reward item projection now has a non-live fixed/selectable/class/extended scaffold composed into the operation plan, but nearby refresh must still treat quest completion as non-production because reward XML loading, live inventory mutation, callbacks, persistence, and packet sends remain incomplete.
