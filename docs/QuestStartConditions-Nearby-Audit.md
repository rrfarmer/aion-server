...
| `com.aionemu.gameserver.questEngine.handlers.AbstractQuestHandler.defaultOnQuestCompletedEvent`; `QuestService.addOrUpdateQuest`; `SM_QUEST_ACTION` | `Aion.GameServer.Services.QuestCompletionFollowUpPlanService`; `Aion.GameServer.Services.QuestCompletionCallbackPlanService` | Callback Follow-Up Dependency | Partial | Unit Tested | Needs Verification | UOW-1028 composes non-live follow-up result plans into callback descriptors after UOW-1027 staged `LOCKED`/`START` outcomes and `ADD`/`UPDATE` packet-action intent. C# still lacks Java start-condition execution, XML condition recursion, live quest-state mutation, packet sends, and quest-finish nested-payload regression coverage. |
