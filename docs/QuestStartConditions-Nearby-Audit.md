...
- Total blocked artifacts: 6 blocked/not-started categories, including production quest template loading/config plumbing, quest-finish composed operation-plan/packet/persistence wiring, NPC faction mentor-title/daily-assignment side effects, timestamp timezone verification, broader refresh-plan archetype audits, and production send triggers
...
Create a staged quest-finish operation plan that composes existing quest-state and NPC faction completion helpers into ordered descriptors for future packet update, callback dispatch, nearby refresh, and persistence. Keep packet sends, production integration, DAO writes, live nearby refresh, and production ItemPurification dispatch disabled until each dependency has tests.
