...
| `com.aionemu.gameserver.network.aion.serverpackets.SM_QUEST_ACTION`; `QuestEngine.onQuestCompleted`; `PlayerQuestListDAO.store`; `PlayerNpcFactionsDAO.storeNpcFactions` | `Aion.GameServer.Services.QuestPersistencePlanService`; `Aion.GameServer.Services.NpcFactionPersistencePlanService`; future C# quest-finish operation plan / packet / persistence boundaries | Packet / Callback / Repository Dependency | Partial | Unit Tested for quest and NPC-faction persistence planning | Needs Verification | UOW-1023 adds non-live NPC-faction persistence planning for Java insert/update filters after UOW-1022 staged quest persistence planning. C# still lacks live callback runtime, DAO writes, Java persistent-state tracking in snapshots, operation-plan persistence composition, and runtime comparison. |
