...
| `com.aionemu.gameserver.network.aion.serverpackets.SM_QUEST_ACTION`; `QuestEngine.onQuestCompleted`; `PlayerQuestListDAO.store`; `PlayerNpcFactionsDAO.storeNpcFactions` | Future C# quest-finish operation plan / packet / persistence boundaries | Packet / Callback / Repository Dependency | Partial | Manual Only | Needs Verification | UOW-1020 documents Java completion callback registration and dispatch ordering. C# still has only descriptors for callback and persistence; live callback runtime, DAO writes, and Java runtime comparison remain missing. |
