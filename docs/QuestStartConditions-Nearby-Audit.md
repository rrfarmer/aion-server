...
| `com.aionemu.gameserver.network.aion.serverpackets.SM_QUEST_ACTION`; `QuestEngine.onQuestCompleted`; `PlayerQuestListDAO.store`; `PlayerNpcFactionsDAO.storeNpcFactions` | `Aion.GameServer.Services.QuestPersistencePlanService`; future C# quest-finish operation plan / packet / persistence boundaries | Packet / Callback / Repository Dependency | Partial | Unit Tested for quest persistence planning | Needs Verification | UOW-1022 adds non-live quest persistence planning for Java delete/insert/update phases. C# still lacks live callback runtime, DAO writes, NPC-faction persistence planning, Java persistent-state tracking in snapshots, and runtime comparison. |
