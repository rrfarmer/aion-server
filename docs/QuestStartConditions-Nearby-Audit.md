...
- Total artifacts ported: 15 staged partial artifacts across UOW-992 through UOW-1012 (`NearbyQuestTemplateTable`, `NearbyQuestStartConditionService`, `NearbyQuestTemplateXmlExtractor`, `NearbyQuestMarkerProjectionService`, `NearbyQuestRefreshPlanService`, `NearbyQuestXmlStartCondition`, `NearbyQuestFinishedCondition`, `NearbyQuestInventoryItem`, `PlayerQuestState` reward/repeat fields, repository hydration, combine-skill predicate support, NPC faction snapshot support, master required-count support, NPC faction static-data loading, and staged quest-finish state mutation)
- Total artifacts with verified parity: 0 in this unit
- Total artifacts needing verification: 14
- Total blocked artifacts: 6 blocked/not-started categories, including production quest template loading/config plumbing, quest-finish persistence/packet/callback wiring, NPC faction completion mutation, timestamp timezone verification, broader refresh-plan archetype audits, and production send triggers
...
Audit and stage the NPC faction completion side of `QuestService.finishQuest`, especially `player.getNpcFactions().completeQuest(template)`, or perform the packet/persistence ordering audit for quest completion. Keep packet sends, production integration, DAO writes, live nearby refresh, and production ItemPurification dispatch disabled until each dependency has tests.
