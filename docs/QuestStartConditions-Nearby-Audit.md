...
- XML start-condition semantics are partial: the nearby supported subset is unit-tested, but production static-data loading, unknown future XML children, configurable master-crafting adjustment, and Java runtime comparison remain unverified.
...
- Total artifacts ported: 12 staged partial artifacts across UOW-992 through UOW-1004 (`NearbyQuestTemplateTable`, `NearbyQuestStartConditionService`, `NearbyQuestTemplateXmlExtractor`, `NearbyQuestMarkerProjectionService`, `NearbyQuestRefreshPlanService`, `NearbyQuestXmlStartCondition`, `NearbyQuestFinishedCondition`, `NearbyQuestInventoryItem`, `PlayerQuestState` reward/repeat fields, repository hydration, combine-skill predicate support, and NPC faction snapshot support)
- Total artifacts with verified parity: 0 in this unit
- Total artifacts needing verification: 12
- Total blocked artifacts: 5 blocked/not-started categories, including production quest template loading, quest-finish repeat-date calculation/timezone verification, NPC faction repository/static-data hydration, configurable master-crafting adjustment, and production send triggers
...
Add configurable master-crafting XML required-count parity in a narrow slice, applying Java `CraftConfig.MAX_MASTER_CRAFTING_SKILLS` behavior for `combine_skillpoint == 499`. Keep packet sends, production integration, and production ItemPurification dispatch disabled until each dependency has tests.
