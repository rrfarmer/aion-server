...
- XML start-condition semantics are partial: the nearby supported subset is unit-tested, but production static-data loading, unknown future XML children, configurable master-crafting adjustment, NPC-faction dependencies, and Java runtime comparison remain unverified.
...
- NPC faction, abyss-rank/title/class/race/gender enum mapping, exception/log behavior, and production static-data loading need C# homes before runtime candidate filtering can be claimed.
...
- Total artifacts ported: 11 staged partial artifacts across UOW-992 through UOW-1003 (`NearbyQuestTemplateTable`, `NearbyQuestStartConditionService`, `NearbyQuestTemplateXmlExtractor`, `NearbyQuestMarkerProjectionService`, `NearbyQuestRefreshPlanService`, `NearbyQuestXmlStartCondition`, `NearbyQuestFinishedCondition`, `NearbyQuestInventoryItem`, `PlayerQuestState` reward/repeat fields, repository hydration, and combine-skill predicate support)
- Total artifacts with verified parity: 0 in this unit
- Total artifacts needing verification: 11
- Total blocked artifacts: 5 blocked/not-started categories, including production quest template loading, quest-finish repeat-date calculation/timezone verification, NPC faction dependencies, configurable master-crafting adjustment, and production send triggers
...
Add the next missing nearby predicate dependency in a narrow slice: NPC faction checks, or broader refresh-plan audits across representative player archetypes. The NPC faction audit recommends a staged faction snapshot first, with active exact-faction checks and mentor/non-mentor slot cooldown behavior. Keep packet sends, production integration, and production ItemPurification dispatch disabled until each dependency has tests.
