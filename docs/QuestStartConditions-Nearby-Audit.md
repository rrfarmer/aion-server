...
Date: May 25, 2026
Unit of Work: UOW-991, updated by UOW-992 through UOW-1002
...
|---|---|---|---|---|---|---|
| `com.aionemu.gameserver.services.QuestService.checkStartConditions`; `QuestService.inventoryItemCheck`; `QuestState.canRepeat` | `Aion.GameServer.Services.NearbyQuestStartConditionService` | Service / Quest Predicate | Partial | Unit Tested | Partial Parity | Stages nearby UI early gates, XML subset, inventory item-id presence checks, and UOW-1002 repeat timing for completed quests. Combine-skill checks, NPC faction checks, warning packets, exception/log behavior, production static-data loading, and live send triggers remain unsupported. |
...
- NPC faction, combine-skill, abyss-rank, title, class/race/gender enum mapping, exception/log behavior, and production static-data loading need C# homes before runtime candidate filtering can be claimed.
...
- Total artifacts ported: 10 staged partial artifacts across UOW-992 through UOW-1002 (`NearbyQuestTemplateTable`, `NearbyQuestStartConditionService`, `NearbyQuestTemplateXmlExtractor`, `NearbyQuestMarkerProjectionService`, `NearbyQuestRefreshPlanService`, `NearbyQuestXmlStartCondition`, `NearbyQuestFinishedCondition`, `NearbyQuestInventoryItem`, `PlayerQuestState` reward/repeat fields, and repository hydration)
- Total artifacts with verified parity: 0 in this unit
- Total artifacts needing verification: 11
- Total blocked artifacts: 5 blocked/not-started categories, including production quest template loading, quest-finish repeat-date calculation/timezone verification, NPC faction/combine-skill dependencies, and production send triggers
...
Add the next missing nearby predicate dependency in a narrow slice: combine-skill checks, NPC faction checks, or broader refresh-plan audits across representative player archetypes. Keep packet sends, production integration, and production ItemPurification dispatch disabled until each dependency has tests.
