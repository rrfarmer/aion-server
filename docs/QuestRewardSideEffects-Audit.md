...
- UOW-1073 adds `QuestXpCustomRewardRuntimeInputAdapterService`, a disabled-by-default adapter that can execute bonus/faction custom reward boundaries and return `QuestXpLevelChangeContextFactoryInput` with supplied execution results.
...
- Existing XP level-change sub-plan prerequisite:
  ...
  - `QuestXpCustomRewardRuntimeInputAdapterService`
  - `QuestFinishCustomRewardRuntimeSideEffectAdapterService`
  - `QuestFinishRuntimeInput-Audit.md`
...
1. Extend static reward extraction to regular `reward_item` and `selectable_reward_item` lists while keeping class rewards, extended rewards, and bonus handlers disabled.
