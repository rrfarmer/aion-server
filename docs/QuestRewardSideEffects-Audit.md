...
1. Run the opt-in system-mail DB integration against a live MySQL schema when available, add failure-ordering DB coverage, or add a disabled-by-default runtime adapter that gathers custom reward execution inputs for XP level-change composition.
