...
1. Add a disabled quest-finish/session assembly adapter that reads `Player.AccountCreationEpochMillis`, `IDFactory.NextId`, and static item templates into `QuestFinishCustomRewardRuntimeInputAssemblerService` without enabling live custom reward execution.
