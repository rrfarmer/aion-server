...
1. Add a concrete opt-in executor for `SystemMailRewardPersistenceExecutionService` that bridges `MailDAO`/`InventoryDAO` SQL and online mailbox fanout, or add a disabled-by-default runtime adapter that gathers custom reward execution inputs for XP level-change composition.
