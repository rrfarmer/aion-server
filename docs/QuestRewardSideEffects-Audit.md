...
1. Run the opt-in system-mail DB integration suite against a live MySQL schema when available, add online mailbox fanout packet-order coverage, or add a disabled-by-default runtime adapter that gathers custom reward execution inputs for XP level-change composition.
