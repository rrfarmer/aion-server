...
- Next implementation slice should continue deeper equipment stat effects such as godstone combat proc metadata, armor mastery, the remaining idian/conditioning service burn/update packet flow, housing auction settlement/maintenance/sign/appearance flows, full known-list fanout/movement broadcast, or broader chat packet surfaces.
...
1. Deepen equipment/item stat parity from the Java source: godstone combat proc metadata, armor mastery, remaining idian/conditioning service burn/update flow, and the eventual `CreatureGameStats`/`Stat2` container.
