...
- Next implementation slice should start from the remaining 6E queue: NPC/dialog equipped-item charge-all confirmation/payment flow, exact item-use delay/cancel observers, charge/idian burn trigger integration through future skill/combat observers, armor mastery after a skill/effect strategy is chosen, equip/unequip recompute fanout, housing auction settlement/maintenance/sign/appearance flows, or persistent known-list membership.
...
- [ ] Inventory/equipment load and stat application (partial: typed inventory rows and `item_stones` rows loaded; item-stone packet display is implemented; first-pass equipped item template weapon stats/modifiers, socketed mana/fusion stone modifiers, fusioned weapon modifiers/10% bonuses, selected random bonus modifiers, item sets, enchantment, tempering, conditioning, and idian POLISH stats now apply in `SM_STATS_INFO`; selected cube-item conditioning service/payment/update flow is implemented; godstone effects, armor mastery, effect/stat-container parity, equip/unequip recompute, charge burn, and idian polish mutation remain pending)
