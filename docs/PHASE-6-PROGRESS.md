...
- Next implementation slice should continue broker class/recipe filters, visible postman spawn/known-list support, housing auction/bid flows, or equipment stat application once item templates/stat functions are in scope.
...
- [x] Register and parse broker client opcodes `117`, `123`-`130`; DB-backed responses exist for sell-window price range, registered items, settled items, item-ID search, numeric category-mask list/search, cancel-registered item return, settle-account collection, register-item persistence, and buy-item persistence, while class/recipe filters remain pending
