...
- XP extraction item-template metadata now parses Java `<actions><expextract item_id percent cost/>` into `ItemTemplateSummary` so the next `ExpExtractAction` runtime slice can use Java's reward item and fixed/percent experience-cost values.
...
1. Port Java `ExpExtractAction` runtime using `ItemTemplateSummary.ExpExtractAction`.
2. Continue tightening reusable item-add behavior with richer Java partial-add caller messaging, then reuse it for future reward/remodel/assembly flows.
