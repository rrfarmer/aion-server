...
- [ ] Inventory/equipment load and stat application (partial: typed inventory rows and `item_stones` rows loaded; item-stone packet display is implemented; first-pass equipped item template weapon stats/modifiers plus socketed mana/fusion stone modifiers now apply in `SM_STATS_INFO`; random bonuses, item sets, enchantment, tempering, conditioning, idian/godstone effects, and effect/stat-container parity remain pending)
...
1. Deepen equipment/item stat parity from the Java source: fusion weapon item stats, random bonus modifiers, item sets, enchantment, tempering, conditioning, idian/godstone effects, armor mastery, and the eventual `CreatureGameStats`/`Stat2` container.
