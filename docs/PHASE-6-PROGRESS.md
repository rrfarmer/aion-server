...
- ItemTemplateSummary now also parses Java `attack_type`, `<weapon_stats>`, stat bonus set IDs, enchant template names, and direct item stat modifiers; `SM_STATS_INFO` applies first-pass equipped item template weapon stats/modifiers, socketed mana/fusion stone template modifiers, fusioned weapon template modifiers/10% weapon stat bonuses, selected random bonus modifiers, Java item-set part/full bonuses, and Java enchant-template stat effects to current stats on login while full Java stat-container parity remains pending.
...
- Next implementation slice should continue deeper equipment stat effects such as tempering/conditioning/idian/godstone effects, housing auction settlement/maintenance/sign/appearance flows, full known-list fanout/movement broadcast, or broader chat packet surfaces.
...
- [ ] Inventory/equipment load and stat application (partial: typed inventory rows and `item_stones` rows loaded; item-stone packet display is implemented; first-pass equipped item template weapon stats/modifiers, socketed mana/fusion stone modifiers, fusioned weapon modifiers/10% bonuses, and selected random bonus modifiers now apply in `SM_STATS_INFO`; item sets, enchantment, tempering, conditioning, idian/godstone effects, and effect/stat-container parity remain pending)
...
1. Deepen equipment/item stat parity from the Java source: tempering, conditioning, idian/godstone effects, armor mastery, and the eventual `CreatureGameStats`/`Stat2` container.
