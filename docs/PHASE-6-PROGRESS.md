...
- ItemTemplateSummary now also parses Java `attack_type`, `<weapon_stats>`, and direct item stat modifiers; `SM_STATS_INFO` applies first-pass equipped item template weapon stats/modifiers, socketed mana/fusion stone template modifiers, and fusioned weapon template modifiers/10% weapon stat bonuses to current stats on login while full Java stat-container parity remains pending.
...
- [ ] Inventory/equipment load and stat application (partial: typed inventory rows and `item_stones` rows loaded; item-stone packet display is implemented; first-pass equipped item template weapon stats/modifiers, socketed mana/fusion stone modifiers, and fusioned weapon modifiers/10% bonuses now apply in `SM_STATS_INFO`; random bonuses, item sets, enchantment, tempering, conditioning, idian/godstone effects, and effect/stat-container parity remain pending)
...
1. Deepen equipment/item stat parity from the Java source: random bonus modifiers, item sets, enchantment, tempering, conditioning, idian/godstone effects, armor mastery, and the eventual `CreatureGameStats`/`Stat2` container.
