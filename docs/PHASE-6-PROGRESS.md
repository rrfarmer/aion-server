...
- Next implementation slice should start from the remaining 6E queue: NPC/dialog equipped-item charge-all confirmation/payment flow, exact item-use delay/cancel observers, charge/idian burn trigger integration through future skill/combat observers, broader skill/effect stat strategy beyond armor mastery, equip/unequip recompute fanout, housing auction settlement/maintenance/sign/appearance flows, or persistent known-list membership.
...
1. Port NPC/dialog equipped-item charge-all confirmation/payment flow: `ItemChargeService.startChargingEquippedItems`, `SM_QUESTION_WINDOW`, response handling, and kinah/AP prepayment.
...
3. Choose the next skill/effect stat slice beyond armor mastery: broader passive effect parsing, title/effect stats, or a fuller Java `Stat2` bridge.
