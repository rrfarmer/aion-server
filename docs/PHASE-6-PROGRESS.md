...
- Next implementation slice should start from the remaining 6E queue: exact item-use delay/cancel observers, charge/idian burn trigger integration through future skill/combat observers, broader skill/effect stat strategy beyond armor mastery, equip/unequip recompute fanout, housing auction settlement/maintenance/sign/appearance flows, persistent known-list membership, or full NPC/dialog known-list/function validation.
- Latest validation: `dotnet test dotnetConversion\AionServer.slnx --no-restore` passed with 337 tests.
...
- [ ] Inventory/equipment load and stat application (partial: typed inventory rows and `item_stones` rows loaded; item-stone packet display is implemented; first-pass equipped item template weapon stats/modifiers, socketed mana/fusion stone modifiers, fusioned weapon modifiers/10% bonuses, selected random bonus modifiers, item sets, enchantment, tempering, conditioning, idian POLISH stats, and learned armor mastery stats now apply in `SM_STATS_INFO`; selected cube-item conditioning service/payment/update flow, equipped item charge-all confirmation/payment flow, charge consumable flow, and idian polish mutation are implemented; godstone effects, full skill/effect stat-container parity, equip/unequip recompute, and charge/idian burn observers remain pending)
...
2. Choose the next skill/effect stat slice beyond armor mastery: broader passive effect parsing, title/effect stats, or a fuller Java `Stat2` bridge.
3. Port equip/unequip behavior and stat refresh side effects, including `SM_STATS_INFO` recompute/fanout and speed/emotion updates where Java sends them.
