...
- `CM_EQUIP_ITEM` now parses Java `restrict`, `restrict_max`, and `<uselimits gender/rank>` metadata, rejects invalid class, low-level, high-level, race, gender, AP-rank, and missing required equip-skill attempts, and keeps the first-pass equip/unequip/switch persistence, stats refresh, and appearance fanout.
...
1. Finish full `CM_EQUIP_ITEM` parity beyond the current mutation and guard path: stigma handling, soul-bind confirmation, identify checks, power-shard emotion side effects, quest/summon observers, and exact speed/emotion fanout.
