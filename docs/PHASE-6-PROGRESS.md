...
- ItemTemplateSummary now also parses Java `attack_type`, `<weapon_stats>`, and direct item stat modifiers; `SM_STATS_INFO` applies first-pass equipped item template weapon stats/modifiers to current stats on login while full Java stat-container parity remains pending.
...
- [ ] Inventory/equipment load and stat application (partial: typed inventory rows and `item_stones` rows loaded; item-stone packet display is implemented; first-pass equipped item template weapon stats/modifiers now apply in `SM_STATS_INFO`; mana stones, random bonuses, item sets, enchantment, tempering, conditioning, and effect/stat-container parity remain pending)
