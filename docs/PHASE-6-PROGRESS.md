...
- Next implementation slice should continue broker class/recipe filters and buy mutation, visible postman spawn/known-list support, housing auction/bid flows, or equipment stat application once item templates/stat functions are in scope.
- Latest validation: `dotnet test dotnetConversion\AionServer.slnx` passed with 263 tests.
...
- [x] Register and parse broker client opcodes `117`, `123`-`130`; DB-backed responses exist for sell-window price range, registered items, settled items, item-ID search, numeric category-mask list/search, cancel-registered item return, settle-account collection, and register-item persistence, while class/recipe filters plus broker buy mutation remain pending
...
1. Continue broker class/recipe filters and buy mutation, visible postman spawn/known-list support, or housing auction/bid flows.
