...
- Next implementation slice should continue deeper equipment stat effects such as idian/godstone effects, armor mastery, the remaining conditioning service/payment/burn/update packet flow, housing auction settlement/maintenance/sign/appearance flows, full known-list fanout/movement broadcast, or broader chat packet surfaces.
...
1. Deepen equipment/item stat parity from the Java source: idian/godstone effects, armor mastery, remaining conditioning service/payment/burn/update flow, and the eventual `CreatureGameStats`/`Stat2` container.
