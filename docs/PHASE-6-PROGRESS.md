...
- Next implementation slice should start from the remaining 6E queue: exact item-use delay/cancel observers, charge/idian burn trigger integration through future skill/combat observers, broader skill/effect stat strategy beyond armor mastery/title modifiers, equip/unequip recompute fanout, housing auction settlement/maintenance/sign/appearance flows, persistent known-list membership, or full NPC/dialog known-list/function validation.
- Latest validation: `dotnet test dotnetConversion\AionServer.slnx --no-restore` passed with 338 tests.
...
2. Choose the next skill/effect stat slice beyond armor mastery and bonus titles: broader passive effect parsing, weapon/shield mastery, or a fuller Java `Stat2` bridge.
