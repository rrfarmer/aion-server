...
- `CM_MANASTONE` opcode `74` now covers Java's stigma charge-stone branch, manastone removal branch, and a first godstone socketing foundation: matching charge stones consume with Java `DEC_STIGMA_USE`, success increments `inventory.enchant`, equipped stigmas refresh temporary stigma skills, failure destroys the stigma and removes its skills, action `3` validates the current NPC target object, removes normal/fusion `item_stones` slots, charges Java's 650 Kinah fee, persists the stone delete/Kinah mutation, and emits Java-shaped remove-option system/inventory packets, while action `4` validates cube-only/non-equipped target weapons, Java `CAN_PROC_ENCHANT` mask, godstone source/template metadata, source consume/delete, target `item_stones` godstone persistence, and Java-shaped proc system/item packets. Exact delayed task cancellation observers and richer NPC known-list/range validation remain pending.
...
2. Continue `CM_MANASTONE` beyond stigma charge/removal/godstone foundation: Java non-stigma manastone socketing, enchant, amplification, supplement validation, delayed use animations, and abort cleanup.
