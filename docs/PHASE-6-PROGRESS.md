...
- `CM_EQUIP_ITEM` now parses Java `restrict`, `restrict_max`, and `<uselimits gender/rank>` metadata, rejects invalid class, low-level, high-level, race, and gender attempts with Java `SM_SYSTEM_MESSAGE` IDs, and keeps the first-pass equip/unequip/switch persistence, stats refresh, and appearance fanout.
...
1. Finish full `CM_EQUIP_ITEM` parity beyond the current mutation and basic guard path: AP-rank guard/l10n, required equip skills, stigma handling, soul-bind confirmation, identify checks, power-shard emotion side effects, quest/summon observers, and exact speed/emotion fanout.
