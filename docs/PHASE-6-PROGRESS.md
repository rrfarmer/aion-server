...
1. Continue item-action routing with Java `DecomposeAction` runtime now that `decomposable_items.xml` and `<decompose/>` metadata are loaded; keep remodel behind NPC/dialog validation unless that support is expanded first.
