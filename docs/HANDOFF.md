...
**Done on branch:** `WorldPosition` struct→class; `RegionUtil`; `IZoneHandler`/`IAdvancedZoneHandler`; **F2 layer** — `KnownObject`, `VisibleObjectController` (non-generic base + generic `<T>` per decision #2), `VisibleObject` (decision #4; `SetTarget` verbatim Java quirk preserved).
**Current error frontier (next to port):** `MapRegion` (8), `Creature` (8), `ZoneInstance` (6), `KnownList` (6), `WorldMapInstance` (4), `World.Geo`/`GeoService` (2), plus `RespawnService` + faithful-`World` member calls (flat-World swap pending).
