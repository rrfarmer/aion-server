...
**Next:** isolated leaves essentially exhausted → the interconnected core (`Skill`/`SkillTemplate`/`Effect`/`EffectTemplate`/`Creature`/`Player`) must port and close together (the convergence; `PacketSendUtility` + Player-connection at F4).
