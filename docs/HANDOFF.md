...
- **✅ substrate progress (2026-06-08):** `Rnd` (commons/utils, full API) now ported at `Commons/Utils/Rnd.cs` — canonical; sites inlining `Random.Shared` (e.g. `SpawnGroup`) can migrate later. `AtomicBoolean` also exists.
...
- **`AggroList` (controllers/attack, 216L)** forward-deps still missing: `Creature`/`SummonedObject`/`Player` (spine), `StatFunctions` (combat formulas), `GeoService` (geo engine), `DamageList` (sibling controllers/attack class — also missing), `DataManager.TRIBE_RELATIONS_DATA` member. `Rnd`/`AggroInfo`/`AggroTarget`/`AbnormalState`/`HopType`/`PositionUtil`/`ThreadPoolManager` ready. Port `DamageList` alongside it.
