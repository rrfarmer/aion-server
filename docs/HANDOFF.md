...
**Done on branch:** `WorldPosition` struct→class; `RegionUtil`; `IZoneHandler`/`IAdvancedZoneHandler`; F2 layer (`KnownObject`, `VisibleObjectController` base+generic, `VisibleObject`); `KnownList` + `PositionUtil` VisibleObject overloads; `ZoneInstance`; **stats engine** — `Stat2`(+`AdditionStat`/`ReverseStat`), `AggroInfo`, **StatFunction cone** (`IStatFunction`, `StatFunction`, `StatAdd/Rate/Sub/Set/Abs Function`, `StatFunctionProxy`).
- ✅ `StatCondition` (interface), `NpcStatCalculation` (clean — all deps satisfied).
**Bottom-up note:** remaining stats fns (`StatArmorMasteryFunction`/`StatWeaponMasteryFunction`/`StatShieldMasteryFunction`/`PlayerStatFunctions`) need `Item`/`Player`/`ItemTemplate`; `StatCapUtil` needs `Creature`/`Player` — port AFTER those. `Conditions` cone needs its ~25 condition subclasses (→`Effect`/`Skill`/`Creature`/`Player`) — blocked.

- ✅ `ChatType` (clean leaf; client id + sysMsg + reverse lookup).
- ✅ skillengine/state enums (9): `DashStatus`,`DispelType`,`FlyingRestriction`,`HealType`,`ProvokeTarget`,`ProvokeType`,`SignetEnum`,`SubEffectType`,`FlyState`.
- ✅ skill-engine data classes (10): `SummonOwner`, `ItemCooldown`, `SkillAliasPosition`/`SkillAliasLocation`, `Times`, `SignetData`/`SignetDataTemplate`, `Motion`, `EffectReserved`(+nested `ResourceType`), `WeaponTypeWrapper`.
- **Tool:** clean-candidate scan (imports vs existing C# types) — MISSES same-package `extends`, so verify. XML data classes use **public properties** (C# XmlSerializer needs public members; Java uses `@XmlAccessorType(FIELD)`). `afterUnmarshal` → an `AfterUnmarshal()` method (string interning etc.).
- **Deferred (blocked):** `SkillLearnTemplate` (`getSkillLevel` → `DataManager.SKILL_DATA.getSkillTemplate().getLvl()` needs `SkillTemplate`); `MotionTime` (needs `Player`); effect subclasses (`AbsoluteSlowEffect`…) extend unported `EffectTemplate`; `ChainSkill(s)`/`ChargedSkill`/`ChargeSkillEntry` — verify vs `Skill`/`SkillTemplate`.
- ✅ combat/AI/team leaves (10): `AttackStatus`,`AggroTarget`,`DamageInfo<T>`,`AttackResult`,`AttackerCriticalStatus`,`AttackIntention`,`AiNameAttribute`(Java @AIName annotation→C# attribute),`TeamType`,`ITeamEvent`,`ITeamMember<out M>`.
- **C# gotcha logged:** an extension method named `GetType()` is shadowed by `Object.GetType()` — Java `getType()` accessors become `GetTypeValue()` (see `TeamType`). Same family as the `getType()`→property cases.
- ✅ skill-cone leaves (5): `ChainSkill`,`ChainSkills`,`ChargedSkill`,`ChargeSkillEntry`,`SubEffect`.
- ✅ base interfaces + model enums (5): `IPersistable`(+`PersistentState`),`IExpirable`,`AttendType`,`DuelResult`,`LetterType`.
- CONFIRMED blocked: `StatShieldMasteryFunction`/`StatWeaponMasteryFunction` (`stat.getOwner()`→`Player.getEquipment()` = Creature-Player). `EmotionId` (large clean enum) still TODO — port next.
- **Next:** remaining isolated leaves nearly gone. Still-clean candidates to consider (verify member usage): model leaves `AttendType`/`DuelResult`/`EmotionId`/`Expirable`/`Persistable`/`LetterType` (spine-relevance varies), `PlayerTeamMember` (team). BLOCKED/verify: `MotionTime`+mastery stat fns (`StatShieldMasteryFunction`/`StatWeaponMasteryFunction`) use flat-`Player` members that may not exist → check; `Effects` is an @XmlElements wrapper over the ~170 effect subclasses (needs `EffectTemplate`); `SkillLearnTemplate` needs `SkillTemplate`. The interconnected core (`Skill`/`SkillTemplate`/`Effect`/`EffectTemplate`/`Creature`) lands together.
