...
- **Next:** continue harvesting any remaining clean leaves, but the cone is converging on the interconnected core (`Skill`/`SkillTemplate`/`Effect`/`EffectTemplate`/`Creature`) which must land together.
