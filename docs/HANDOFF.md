...
**Done on branch:** `WorldPosition` struct→class; `RegionUtil`; `IZoneHandler`/`IAdvancedZoneHandler`; F2 layer (`KnownObject`, `VisibleObjectController` base+generic, `VisibleObject`); `KnownList` + `PositionUtil` VisibleObject overloads; `ZoneInstance`; **stats-engine start** — `Stat2`(+`AdditionStat`/`ReverseStat`), `AggroInfo`.
