...
**DECISION NEEDED — `Race` enum fidelity (blocks `ItemActivationTarget`):** the existing C# `Race` (commit `cdbfa72a2`) uses **PascalCase** members (`Elyos`,`Brownie`,…) but Java `Race` is `@XmlEnum` (SCREAMING_SNAKE tokens in data) — latent XML-compat risk; AND it **dropped `LIVINGWATER(28)`** (Java has both `LIVINGWATER(28)` and `DEFORM(28)`; C# kept only `Deform`). Options: (a) minimal additive fix — add `Livingwater = 28` (alias), keep PascalCase; (b) full re-port of `Race` to SCREAMING_SNAKE for XML parity (blast radius: referenced in `CharacterCreationRepository`/`PlayerEnterWorldRepository`/`DecomposableItemTable`/…). Until decided, `ItemActivationTarget` (needs `Race.LIVINGWATER`) is deferred.

**Large generatable leaf (own pass):** `model/DialogAction` — 6246-line `final class` of `public static final int` dialog-action ids (converted from enum in Java due to the 64KB enum bytecode limit) + a reflection-based int→name lookup. Zero-dep; generate the constants from Java, port the lookup. Used by `Npc`.

**Item/npc cone nodes blocked on the SCC core (port in/after the big-bang):** `ResultedItem`→`ResultedItemsCollection`/`ExtractedItemsCollection`/`ReturnLocList`/`MultiReturnItem`/`DecomposableItemInfo` (need `Player`+`DataManager`); `Stigma` (needs `SkillTemplate`); `ItemUseLimits` (needs `AbyssRankEnum`→`Player`); `ItemActions` (action-class hierarchy→`Player`/`Item`); `ModifiersTemplate`/`StatFunction` (→`Stat2`→`Creature`); `ItemSetTemplate` (needs `ItemPart`/`PartBonus`/`FullBonus`); `KiskStatsTemplate`✅ but `ItemTemplate` itself still blocked; `CustomConfig` (config class, port like `WorldConfig` when needed).
