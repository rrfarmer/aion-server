...
- **Tool:** a clean-candidate scan (checks each Java file's imports vs existing C# types) is the quickest way to find the next batch — but it MISSES same-package `extends`/refs, so verify (e.g. effect subclasses extend the unported `EffectTemplate` → not actually clean). Next clean data-class candidates to verify+port: `SummonOwner`, `ItemCooldown`, `SkillAliasLocation`/`SkillAliasPosition`, `Times`, `Motion`/`MotionTime`, `SignetData`(+Template), `SkillLearnTemplate`, `WeaponTypeWrapper`, `EffectReserved`, `ChainSkill`(s)/`ChargedSkill`.
