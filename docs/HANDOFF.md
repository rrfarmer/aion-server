...
**Bottom-up note:** remaining stats fns (`StatArmorMasteryFunction`/`StatWeaponMasteryFunction`/`StatShieldMasteryFunction`/`PlayerStatFunctions`) need `Item`/`Player`/`ItemTemplate` — port AFTER those (don't port consumers before deps). `StatFunction` references `Conditions` (skill-engine) — forward.
**Next bottom-up targets:** skill-engine `Conditions` cone (→`Effect`/`Skill`) and/or stats containers (`CreatureGameStats`/`CreatureLifeStats` → need `Creature`). The deep engines (Effect/Skill/AI/Creature) are now the gating interconnected core.
