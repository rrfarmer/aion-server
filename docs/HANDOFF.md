...
- **`AggroList` (controllers/attack, 216L)** forward-deps still missing: `Creature`/`SummonedObject`/`Player` (spine), `StatFunctions` (combat formulas), `GeoService` (geo engine), `DataManager.TRIBE_RELATIONS_DATA` member. `Rnd`/`AggroInfo`/`AggroTarget`/`AbnormalState`/`HopType`/`PositionUtil`/`ThreadPoolManager`/`DamageList`/`DamageInfo` ready.
