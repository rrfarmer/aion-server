...
**⛔ FOUNDATIONAL BLOCKER (shift here next): the packet layer.** The engines (`EffectController`/`CreatureController`/`Skill`/`Effect`/`TransformModel`/`CreatureMoveController`) all call `PacketSendUtility.broadcastPacket(new SM_X(...))`. In C# the ~200 `SM_`/`CM_` packets are **fused into the `GameServerConnection` god-class** — there is NO `AionServerPacket` base, `PacketSendUtility`, or standalone `SM_` classes. So before the engines can be ported faithfully, port the packet substrate: `AionServerPacket` (base writer) → `PacketSendUtility` → the specific `SM_` packets each engine sends (de-god-classed from `GameServerConnection`, faithful Java packet structure). This is the gating work for the whole Tier-B engine cone.
