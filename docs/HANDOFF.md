...
- **🔭 ObserveController cone started (no packets/singletons — clean SCC entry):** ported observer bases `ActionObserver` (89L) + `AttackCalcObserver` (63L). Remaining observer subclasses before `ObserveController` (270L): `AttackShieldObserver`/`AttackStatusObserver`/`AttackerCriticalStatusObserver`/`DeathObserver`/`DialogObserver`/`FlyRingObserver`/`ItemUseObserver`/`RoadObserver`/`ShieldObserver`/`StanceObserver`/`StartMovingListener`/`AbstractCollisionObserver`+actors. Then `ObserveController` itself (forward-refs `Creature`/`Player`/`Item`/`Effect`/`Skill`). `ObserverType`/`AttackerCriticalStatus`/`ShieldType` already ported.
