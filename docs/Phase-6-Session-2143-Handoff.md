# Phase 6 Session 2143 Handoff - FindGroup Action 8 Direct Packet Trace
...
- Actions `0`, `4`, and `8` have disabled-boundary-plus-opt-in direct-packet execution trace evidence.
...
This UOW added focused readiness evidence for Java `CM_FIND_GROUP` action `8`:

- Java action `8` calls `FindGroupService.registerInstanceGroup(player, instanceMaskId, message, minMembers)`.
- Java `registerInstanceGroup` stores a `ServerWideGroup` keyed by player object id and sends `new SM_FIND_GROUP(14, List.of(instanceGroup))` directly to the triggering player.
- C# disabled boundary composes action `8` as a direct `SmFindGroup` action `14` intent.
- A focused test records disabled action `8` acceptance before opt-in registry direct-packet execution to the active player.
- Readiness reports now distinguish action `0`/`4`/`8` disabled direct-packet trace evidence from still-missing live `ProcessPacketAsync` direct-packet proof.

## Java Artifacts Touched

- `com.aionemu.gameserver.network.aion.clientpackets.CM_FIND_GROUP`
- `com.aionemu.gameserver.services.findgroup.FindGroupService`
- `com.aionemu.gameserver.services.findgroup.FindGroupService.registerInstanceGroup`
...
- No live `ProcessPacketAsync` trace proves actions `0`, `4`, or `8` direct-packet ordering relative to a triggering live `CM_FIND_GROUP` client packet.
...
```text
[Phase 6][UOW-2143] Add find group action eight direct trace
```
