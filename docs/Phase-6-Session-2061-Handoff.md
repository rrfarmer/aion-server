# Phase 6 Session 2061 Handoff - Find Group Action 26 Mask Planning
...
- `FindGroupClientActionPlanService` maps parsed client action data into disabled planner calls for Java `CM_FIND_GROUP.runImpl` actions.
- Action `10` composition can now include the optional action `26` enable-register mask-list intent when caller-supplied config/data says Java would send it.
- Action `13` remains an update route and does not plan action `26`, matching Java `!isUpdate`.
...
## Validation In UOW-2061

- Focused C# `FindGroupClientActionPlanServiceTests`, existing `FindGroupRecruitmentPlanServiceTests`, and `SmFindGroupTests` passed:
  - 44 tests passed.
- Focused Java `SM_FIND_GROUP_GoldenTest` and `CM_FIND_GROUP_ReadPayloadGoldenTest` passed:
  - 25 tests passed.
...
- Live `GroupConfig.FORM_INSTANCE_GROUP_ANYWHERE`, target NPC, and `DataManager.AUTO_GROUP` sourcing remain unimplemented.
- Java `showInstanceGroups(Player, Npc portalNpc)`, prepare-window actions, ban action, logout cleanup, and live handler composition remain unported or unverified.

## Next Recommended Unit of Work

- Next sequential task: inspect `FindGroupService.showInstanceGroups(Player, Npc portalNpc)` as a disabled portal-specific action `26` plan boundary.
