# Phase 6 Session 2148 Handoff - FindGroup Action 9 Direct Packet Trace
...
- Actions `0`, `4`, `8`, `9`, `10`, `11`, `13`, and `15` have disabled-boundary-plus-opt-in direct-packet execution trace evidence.
- Action `9` existing and missing removal outcomes both record action `10` updated show-list direct packet execution after disabled boundary acceptance.
...
This UOW added focused readiness evidence for Java `CM_FIND_GROUP` action `9`:

- Java action `9` reads `playerOrTeamId` and `instanceMaskId`, then calls `FindGroupService.removeInstanceGroup(player)`.
- Java `removeInstanceGroup` removes the active player's instance-group registration and always calls `showInstanceGroups(player, true)`.
- Java sends `new SM_FIND_GROUP(10, instanceGroups)` directly to the active player whether or not a registration was removed.
- C# disabled boundary composes action `9` removed and missing outcomes as direct `SmFindGroup` action `10` show-list intents.
- Focused tests record disabled action `9` acceptance before opt-in registry direct-packet execution for both removed and missing outcomes.
- Readiness reports now distinguish action `0`/`4`/`8`/`9`/`10`/`11`/`13`/`15` disabled direct-packet trace evidence from still-missing live `ProcessPacketAsync` direct-packet proof.

## Java Artifacts Touched

- `com.aionemu.gameserver.network.aion.clientpackets.CM_FIND_GROUP`
- `com.aionemu.gameserver.services.findgroup.FindGroupService`
- `com.aionemu.gameserver.services.findgroup.FindGroupService.removeInstanceGroup`
...
- No live `ProcessPacketAsync` trace proves actions `0`, `4`, `8`, `9`, `10`, `11`, `13`, or `15` direct-packet ordering relative to a triggering live `CM_FIND_GROUP` client packet.
...
```text
[Phase 6][UOW-2148] Add find group action nine direct trace
```
