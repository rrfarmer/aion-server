# Phase 6 Session 2145 Handoff - FindGroup Action 15 Direct Packet Trace
...
- Actions `0`, `4`, `8`, `11`, and `15` have disabled-boundary-plus-opt-in direct-packet execution trace evidence.
...
This UOW added focused readiness evidence for Java `CM_FIND_GROUP` action `15`:

- Java action `15` reads `playerOrTeamId` and `instanceMaskId`, then calls `FindGroupService.showInstanceGroupMembersInfo(player, playerOrTeamId)`.
- Java `showInstanceGroupMembersInfo` finds the registered instance group by `playerOrTeamId` and sends `new SM_FIND_GROUP(16, List.of(instanceGroup))` directly to the triggering player when found.
- C# disabled boundary composes action `15` as a direct `SmFindGroup` action `16` member-info intent to the active viewer.
- A focused test records disabled action `15` acceptance before opt-in registry direct-packet execution to the viewer.
- Readiness reports now distinguish action `0`/`4`/`8`/`11`/`15` disabled direct-packet trace evidence from still-missing live `ProcessPacketAsync` direct-packet proof.

## Java Artifacts Touched

- `com.aionemu.gameserver.network.aion.clientpackets.CM_FIND_GROUP`
- `com.aionemu.gameserver.services.findgroup.FindGroupService`
- `com.aionemu.gameserver.services.findgroup.FindGroupService.showInstanceGroupMembersInfo`
...
- No live `ProcessPacketAsync` trace proves actions `0`, `4`, `8`, `11`, or `15` direct-packet ordering relative to a triggering live `CM_FIND_GROUP` client packet.
...
```text
[Phase 6][UOW-2145] Add find group action fifteen direct trace
```
