...
Next, add a disabled-by-default runtime input assembler that refuses enabled custom reward execution when account creation time, item templates, or object-id allocation are missing, and converts login-server account creation milliseconds through configured server time. Keep live XP mutation disabled until stat updates, live nearby quest refresh, live quest handler execution, live skill mutation/effects/recipe learning, live guide HTML send/persistence, live starter-kit/custom reward mail sends, live NPC faction mutation, custom reward DAO writes, and persistence behavior are modeled.
