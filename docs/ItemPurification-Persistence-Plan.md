...
Implementation status as of UOW-965: the method signature, empty-repository recording stub, MySQL repository method, pure persistence payload mapper, inserted-item `item_stones` row persistence, an explicit opt-in persistent live execution seam, a handler-level opt-in helper, handler-level repository-save failure-ordering coverage, an automatic-dispatch readiness policy, a staged dispatch-failure policy, a Java observer design for future packet/DB capture, and opt-in game-server MySQL integration tests exist. The opt-in happy-path and rollback tests passed against a Docker-hosted Java-shaped `aion_gs` schema with `AION_GAMESERVER_DB_INTEGRATION=1`. Automatic `HandleInfrastructurePacketAsync` invocation remains disabled, Java observer artifacts are not generated yet, and quest/AP side-effect execution remains incomplete.
...
- Failure-ordering test/design: handler-level coverage now proves live packet send/mutation succeeds before repository persistence failure is reported. Any automatic dispatch path still needs a deliberate runtime policy because no rollback exists.
...
- Opt-in game-server MySQL test: `PlayerEnterWorldRepositoryDatabaseIntegrationTests.SaveItemPurificationMutation_WritesInventoryStonesAndAbyssRankAgainstJavaSchema_WhenEnabled` seeds Java-shaped rows and asserts material update/delete, base delete, target insert, inherited `item_stones`, and AP rank writes when `AION_GAMESERVER_DB_INTEGRATION=1`.
- Opt-in game-server MySQL rollback test: `PlayerEnterWorldRepositoryDatabaseIntegrationTests.SaveItemPurificationMutation_RollsBackPriorUpdatesWhenRequiredDeleteFailsAgainstJavaSchema_WhenEnabled` forces a missing required delete after a material count update and verifies the C# transaction rolls the earlier update back.
