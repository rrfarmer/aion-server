...
| `com.aionemu.gameserver.services.teleport.BindPointTeleportService` | none; uses distance/client-price reconciliation plus direct requirement checks | `Aion.GameServer.Services.BindPointTeleportPricePlanService`; `Aion.GameServer.Services.BindPointTeleportRequirementsPlanService`; `Aion.GameServer.Services.BindPointTeleportOperationPlanService` | Partial | Medium/High | Analyze `SM_BIND_POINT_TELEPORT` packet/opcode before live scheduling or Kinah mutation. | UOW-1195 added pure `calculateTeleportationPrice` planning. UOW-1196 added pure `checkRequirements` guard ordering. UOW-1197 composed non-live operation intent. Cooldown storage/scheduling, packets, Kinah mutation, and final movement remain separate. |
...
| `com.aionemu.gameserver.services.teleport.BindPointTeleportService` | `Aion.GameServer.Services.BindPointTeleportPricePlanService` / `Aion.GameServer.Services.BindPointTeleportRequirementsPlanService` / `Aion.GameServer.Services.BindPointTeleportOperationPlanService` | Service / Movement | Partial | Unit Tested | Needs Verification | `calculateTeleportationPrice`, `checkRequirements`, and non-live `teleport` operation intent are staged with source-derived unit tests. Cooldown map/threading, delayed task execution, `SM_BIND_POINT_TELEPORT` packet serialization/fanout, Kinah mutation, death recheck, and final movement remain unported. |
...
| `BindPointTeleportOperationPlanServiceTests.CreatePlan_ReadyPlanRecordsJavaSuccessOperationOrder` | Java success operation intent order: start broadcast, 10s task, Kinah decrement, cooldown, cooldown broadcast, final 1s teleport task. | Source-derived only. |
...
- Bind-point teleport price, requirement, and non-live operation planning are staged, but Java cooldown map threading, delayed task execution, `SM_BIND_POINT_TELEPORT` packet serialization/fanout, Kinah mutation, death recheck, and final movement remain unported.
...
1. Analyze and port the concrete `SM_BIND_POINT_TELEPORT` packet/opcode shape in isolation.
