...
| `com.aionemu.gameserver.services.teleport.BindPointTeleportService` | none; uses distance and client-price reconciliation | `Aion.GameServer.Services.BindPointTeleportPricePlanService` | Partial | Medium/High | Add bind-point requirements planner before live scheduling or Kinah mutation. | UOW-1195 added pure `calculateTeleportationPrice` planning: Java float-distance math, long truncation, one-Kinah minimum, client-price max, and warning flag for mismatches over one Kinah. World/race/Kinah/cooldown requirements remain separate. |
...
- Bind-point teleport pricing is staged, but world/race/Kinah/cooldown requirements, Java cooldown map threading, delayed task ordering, and `SM_BIND_POINT_TELEPORT` packet fanout remain unported.
...
1. Add a pure bind-point teleport requirements planner from `BindPointTeleportService.checkRequirements`.
2. Keep teleport price planners non-live until movement routing, packet order, cooldown scheduling, and persistence are ready.
