...
| `com.aionemu.gameserver.services.teleport.BindPointTeleportService` | none; uses distance/client-price reconciliation plus direct requirement checks | `Aion.GameServer.Services.BindPointTeleportPricePlanService`; `Aion.GameServer.Services.BindPointTeleportRequirementsPlanService` | Partial | Medium/High | Compose a non-live bind-point operation plan before live scheduling or Kinah mutation. | UOW-1195 added pure `calculateTeleportationPrice` planning. UOW-1196 added pure `checkRequirements` guard ordering for world, race, Kinah, and cooldown facts. Cooldown storage/scheduling, packets, Kinah mutation, and final movement remain separate. |
...
| `com.aionemu.gameserver.services.teleport.BindPointTeleportService` | `Aion.GameServer.Services.BindPointTeleportPricePlanService` / `Aion.GameServer.Services.BindPointTeleportRequirementsPlanService` | Service / Movement | Partial | Unit Tested | Needs Verification | `calculateTeleportationPrice` and `checkRequirements` are staged with source-derived unit tests. Cooldown map/threading, delayed task scheduling, `SM_BIND_POINT_TELEPORT` packet ordering, Kinah mutation, and final movement remain unported. |
...
- Bind-point teleport price and requirement planning are staged, but Java cooldown map threading, delayed task ordering, `SM_BIND_POINT_TELEPORT` packet fanout, Kinah mutation, and final movement remain unported.
...
1. Compose a non-live bind-point teleport operation plan from price + requirements, still without scheduling or packet sends.
