...
| `com.aionemu.gameserver.services.ArmsfusionService` | `getPriceForService(basePricePerLevelSquared * level * level, race)` | `Aion.GameServer.Services.ArmsfusionPricePlanService` | Partial | Medium | Add `breakWeapons` non-live planner before live dispatch. | UOW-1189 composes the price planner with Java-ordered validation and non-live fusion mutation output. Live DAO store, packet sends, audit logging, and break-weapons remain unported. |
...
| `com.aionemu.gameserver.services.ArmsfusionService` | `Aion.GameServer.Services.ArmsfusionPricePlanService` | Service / Item Mutation Planner | Partial | Unit Tested | Needs Verification | `fusionWeapons` now has a non-live validation/mutation planner with Java failure order, price calculation, kinah deduction output, fuse-item delete/decrement output, fused item fields, tuning reset, and fusion-stone copy. Live DAO store, packet sends, audit logging, and `breakWeapons` remain unported. |
...
1. Add a non-live armsfusion `breakWeapons` planner before live DAO/packet dispatch.
