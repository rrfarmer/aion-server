...
| `com.aionemu.gameserver.services.ArmsfusionService` | `getPriceForService(basePricePerLevelSquared * level * level, race)` | No direct C# `ArmsfusionService` found in this scan | Not Started | Medium | Candidate for next implementation if a planner can be created without live inventory mutation. | Formula is narrow, but armsfusion item mutation and packet fanout must be staged. |
...
1. Add a pure armsfusion price planner if the Java item mutation path can be represented without live side effects.
