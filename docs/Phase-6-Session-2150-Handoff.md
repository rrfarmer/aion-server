# Phase 6 Session 2150 Handoff - FindGroup Action 2 Posted Message Trace
...
- Actions `0`, `2`, `4`, `8`, `9`, `10`, `11`, `13`, `15`, and `17` have disabled-boundary-plus-opt-in direct-packet execution trace evidence.
...
This UOW added focused readiness evidence for Java `CM_FIND_GROUP` action `2`:

- Java action `2` reads `playerOrTeamId`, `message`, and `groupType`, then calls `FindGroupService.addRecruitment(player, message, groupType)`.
- Java ignores the parsed `playerOrTeamId` in `runImpl`; the active player and current team determine the stored recruitment subject.
- Java stores the recruitment before sending packets.
- Java sends `SM_SYSTEM_MESSAGE.STR_PARTY_MATCH_OFFER_PARTY_POSTED()` before calling `showRecruitments(player)`.
- Java `showRecruitments(player)` sends the refreshed `SM_FIND_GROUP` action `0` list directly to the triggering player.
- C# disabled boundary composes action `2` as direct `SmSystemMessage` followed by direct `SmFindGroup` action `0`.
- Focused tests record disabled action `2` acceptance before opt-in registry direct-packet execution and prove posted-message-before-show-list ordering.
- Readiness reports now distinguish action `0`/`2`/`4`/`8`/`9`/`10`/`11`/`13`/`15`/`17` disabled direct-packet trace evidence from still-missing live `ProcessPacketAsync` direct-packet proof.

## Java Artifacts Touched

- `com.aionemu.gameserver.network.aion.clientpackets.CM_FIND_GROUP`
- `com.aionemu.gameserver.services.findgroup.FindGroupService`
- `com.aionemu.gameserver.services.findgroup.FindGroupService.addRecruitment`
...
- No live `ProcessPacketAsync` trace proves actions `0`, `2`, `4`, `8`, `9`, `10`, `11`, `13`, `15`, or `17` direct-packet ordering relative to a triggering live `CM_FIND_GROUP` client packet.
...
```text
[Phase 6][UOW-2150] Add find group action two posted trace
```
