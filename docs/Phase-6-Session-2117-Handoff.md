# Phase 6 Session 2117 Handoff - FindGroup Alliance Join Ordering Evidence
...
- `[Phase 6][UOW-2117] Add find group alliance-join ordering evidence`

## Validation In UOW-2117
...
- Focused C# passed:
  ...
  - Final result: 4 tests passed.
  - Existing nullable/xUnit warnings were emitted from unrelated game-server and test files.
- Focused Java/Maven was not run:
  - No Java source changed. This UOW reviewed Java `PlayerAllianceService.addPlayerToAlliance` and `FindGroupService.onJoinedTeam`; no focused Java test target was identified for this disabled C# recorder evidence.
...
- Broad .NET suite/build was not run in UOW-2117 because no broad-validation trigger applied.
