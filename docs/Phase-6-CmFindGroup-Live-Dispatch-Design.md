...
- Action `6`: posted system message plus refreshed show-list direct packet intents, with focused disabled-boundary execution-order evidence that the posted message is recorded before the refreshed list.
...
- Action `11`: instance-application action 11 direct packet intent to resolved recruiter, with focused disabled-boundary trace evidence that boundary acceptance is recorded before opt-in direct packet execution.
...
- `FindGroupDirectPacketBoundaryTraceReadinessService` now records action `0`/`2`/`4`/`8`/`9`/`10`/`11`/`13`/`15`/`17` disabled-boundary acceptance before opt-in registry execution of direct packets to the Java-selected recipient, while keeping live `ProcessPacketAsync` ordering blocked.
...
- `FindGroupDirectPacketBoundaryTraceReadinessService` records disabled-boundary-plus-opt-in action `0`/`2`/`4`/`8`/`9`/`10`/`11`/`13`/`15`/`17` ordered traces. This is still not live parity because `ProcessPacketAsync` does not invoke the boundary helper or executor for `CmFindGroup`.
...
- Direct packet sends and world broadcasts have opt-in executor ordering evidence, including disabled-boundary action `2` boundary trace, disabled-boundary action `6` executor ordering, and action `10` multi-direct ordering, but still need live connection-registry tests proving packet ordering relative to the triggering client packet.
- Action 12 accepted invite, declined whisper, missing-applicant, and missing-instance-group branches have disabled connection-helper evidence, including the inner planner status surfaced at the connection-boundary intent plan. The accepted group/alliance invites have disabled action `12` boundary-acceptance-before-opt-in-request trace evidence, the declined whisper has focused `SM_MESSAGE` payload evidence, and disabled failure-result tests surface missing direct recipients plus missing invite players without live dispatch or request mutation, but action 12 still needs live boundary tests before being triggered by `CM_FIND_GROUP`.
...
- focused disabled-boundary-plus-opt-in ordered trace for action `6`, proving the posted system message is recorded before the refreshed action `4` show-list packet after boundary acceptance,
