...
- Action 12 accepted invite, declined whisper, missing-applicant, missing-instance-group, and missing invite-runtime branches have disabled connection-helper evidence, including the inner planner status surfaced at the connection-boundary intent plan. The accepted group/alliance invites have disabled action `12` boundary-acceptance-before-opt-in-request trace evidence, the declined whisper has focused `SM_MESSAGE` payload evidence, and disabled failure-result tests surface missing direct recipients, missing invite players, and missing invite runtime without live dispatch or request mutation, but action 12 still needs live boundary tests before being triggered by `CM_FIND_GROUP`.
