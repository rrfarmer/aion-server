...
- `FindGroupDirectPacketBoundaryTraceReadinessService` now records action `0`/`4`/`8`/`9`/`10`/`11`/`13`/`15`/`17` disabled-boundary acceptance before opt-in registry execution of the direct `SmFindGroup` packet to the Java-selected recipient, while keeping live `ProcessPacketAsync` ordering blocked.
...
- `FindGroupDirectPacketBoundaryTraceReadinessService` records disabled-boundary-plus-opt-in action `0`/`4`/`8`/`9`/`10`/`11`/`13`/`15`/`17` ordered traces. This is still not live parity because `ProcessPacketAsync` does not invoke the boundary helper or executor for `CmFindGroup`.
...
- focused disabled-boundary-plus-opt-in ordered traces for action `2` or `6`, proving the posted system message is recorded before the refreshed action `0` show-list packet after boundary acceptance,
