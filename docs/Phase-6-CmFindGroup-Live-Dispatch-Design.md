...
- Action 12 accepted invite, declined whisper, missing-applicant, and missing-instance-group branches have disabled connection-helper evidence, including the inner planner status surfaced at the connection-boundary intent plan. The declined whisper has focused `SM_MESSAGE` payload evidence, but action 12 still needs live boundary tests before being triggered by `CM_FIND_GROUP`.
