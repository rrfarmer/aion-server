...
- `FindGroupDirectPacketBoundaryTraceReadinessService` now records action `0`/`4` disabled-boundary acceptance before opt-in registry execution of the direct `SmFindGroup` packet to the active player, while keeping live `ProcessPacketAsync` ordering blocked.
...
- `FindGroupDirectPacketBoundaryTraceReadinessService` records disabled-boundary-plus-opt-in action `0`/`4` ordered traces. This is still not live parity because `ProcessPacketAsync` does not invoke the boundary helper or executor for `CmFindGroup`.
...
- additional direct-action boundary traces beyond actions `0` and `4`,
