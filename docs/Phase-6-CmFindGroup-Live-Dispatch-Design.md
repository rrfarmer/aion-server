...
- `FindGroupDirectPacketBoundaryTraceReadinessService` now records action `0`/`4`/`8`/`11`/`15` disabled-boundary acceptance before opt-in registry execution of the direct `SmFindGroup` packet to the Java-selected recipient, while keeping live `ProcessPacketAsync` ordering blocked.
...
- `FindGroupDirectPacketBoundaryTraceReadinessService` records disabled-boundary-plus-opt-in action `0`/`4`/`8`/`11`/`15` ordered traces. This is still not live parity because `ProcessPacketAsync` does not invoke the boundary helper or executor for `CmFindGroup`.
...
- additional direct-action boundary traces beyond actions `0`, `4`, `8`, `11`, and `15`,
