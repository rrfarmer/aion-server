# Phase 6 Session 2211 Handoff - FindGroup Mutation Dry-Run Java Row Projection
...
- `FindGroupMutationPostProjectedRowComparisonDryRunContractService` now carries shape-valid Java artifact row references and accepted guarded C# row references as future executor inputs.
- Shape-valid Java artifacts, shape-valid disabled C# rows, guarded fixture skeleton metadata, readiness aggregate metadata, guarded fixture result contract metadata, envelope handoff metadata, and dry-run row references are not verified parity.

## UOW-2211 Summary

This UOW added shape-valid Java artifact row references to the projected-row comparison dry-run contract.

Key behavior:

- adds `AcceptedJavaRows`,
- adds `HasJavaArtifactDirectoryReport`,
- carries Java row action, mutation kind, row identity, artifact-shape flag, evidence, and planned input source,
- keeps default dry-run blocked with no Java or C# row references when no artifacts are supplied,
- allows shape-valid Java rows to be named as future executor inputs only when supplied through the guarded Java artifact directory report,
...
- Shape-valid Java fixture artifacts, disabled C# projection rows, guarded fixture skeleton metadata, readiness aggregate metadata, guarded fixture result contract metadata, envelope handoff metadata, and dry-run row references are not Java/C# runtime comparison evidence.
...
```text
[Phase 6][UOW-2211] Add find group mutation dry-run Java row references
```
