...
- Kept `docs/PHASE-6-PROGRESS.md` current through Session 168.

---

## Important Limits

- Soul-bind stance-denial guards/messages are still pending: dead, ride, chair, resting, gliding, flying, and weapon-equipped checks from Java `Equipment.soulBindItem`.
...
1. Add Java soul-bind stance-denial guard/message parity before the delayed accept path: dead, ride, chair, resting, gliding, flying, and weapon-equipped state checks.
2. Add shared delayed item-use abort cleanup for item cooldowns and any per-player using-item state now that enchant, manastone, godstone, stigma charge, charge action, idian polish, and soul-bind all use the scheduler.
