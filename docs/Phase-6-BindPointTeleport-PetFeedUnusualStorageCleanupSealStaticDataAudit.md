...
5. Continue wiring runtime callers to compute/pass the explicit flag from `StaticData.ItemRestrictionCleanups` where that dependency is available; quest/custom reward paths need a read-only caller audit before broader reward plumbing changes.
