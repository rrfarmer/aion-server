...
5. Continue wiring runtime callers or staged packet metadata to compute/pass the explicit flag from `StaticData.ItemRestrictionCleanups` where that dependency is available; pet-feed normal-cube unlock metadata can use the already precomputed context flag, while quest/custom reward item execution still needs a concrete live item sender before wiring.
