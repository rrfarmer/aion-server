...
5. Continue wiring runtime callers to compute/pass the explicit flag from `StaticData.ItemRestrictionCleanups` where that dependency is available, starting with decompose reward helpers; world loot needs `ItemRestrictionCleanupTable?` passed into `WorldNpcLootService`.
