...
- C# now has the `questUpdateItems` static-data membership projection, a no-op nearby-refresh planner, and a no-op dispatcher seam, but it still lacks `questItems` get-handler registration and any dispatcher that invokes real nearby-quest refresh.
...
- Add a player-controller refresh adapter only after the C# player-controller nearby quest refresh surface is understood; keep it disconnected from production ItemPurification dispatch before invoking any live quest handlers.
