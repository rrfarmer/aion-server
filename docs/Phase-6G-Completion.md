...
**Current validation baseline**: `dotnet test dotnetConversion\AionServer.slnx --no-restore` passes with 361 tests.
...
- Kept `docs/PHASE-6-PROGRESS.md` current through Session 140.

---

## Important Limits

- `CM_EQUIP_ITEM` now covers the mutation path, Java class/level/max-level/race/gender/AP-rank/required-skill/unidentified guards, soul-bind confirmation foundation, and a first-pass normal stigma equip/unequip skill/Kinah bridge.
...
- Stigma linked-skill unlock selection, stigma enchant/charge-stone flow, membership slot overrides, exact hidden-skill message behavior, and full SkillEngine effect application are still pending.
...
1. Continue `CM_EQUIP_ITEM` parity with a consciously scoped branch: linked stigma unlock selection, stigma enchant/charge-stone flow, or exact soul-bind timing/cancel/stance behavior once player state support exists.
2. Tighten StigmaService behavior in slices: membership slot overrides, exact hidden-skill messages, linked-skill add/remove parity, and full Java SkillEngine effect application after temporary skill mutations.
