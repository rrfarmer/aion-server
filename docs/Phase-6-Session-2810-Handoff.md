...
- Live NPC-target `ASK_QUEST_ACCEPT`, `QUEST_ACCEPT`, `QUEST_ACCEPT_1`, `QUEST_ACCEPT_SIMPLE`, `QUEST_REFUSE_1`, `QUEST_REFUSE_2`, and `QUEST_REFUSE_SIMPLE` now execute the directly wired Java start-dialog packet/state slices.
...
- `docs/Phase-6-Session-2810-Completion.md`.
- `docs/Phase-6-Session-2810-Handoff.md`.
...
|---|---|---|---|---|---|---|
| `AbstractQuestHandler.sendQuestStartDialog` `ASK_QUEST_ACCEPT` | `GameServerConnection.TryHandleNpcTargetQuestStartAskAcceptAsync` | Quest dialog handler slice | Partial | Regression Tested | Partial Parity | Sends Java-equivalent ask-accept page for registered NPC-start quests; broader handler dispatch remains incomplete. |
| `DialogAction` | `CmDialogSelect` | Dialog action constants | Partial | Regression Tested | Partial Parity | Adds live-consumed `ASK_QUEST_ACCEPT`; this is not a complete DialogAction port. |
...
Recommended next UOW: `[Phase 6][UOW-2811] Wire NPC-target FINISH_DIALOG quest selection page`

- Deferred/live behavior to advance: execute the Java finish-dialog branch for registered NPC quest-start dialogs.
- Java source of truth: `AbstractQuestHandler.sendQuestStartDialog` handles `FINISH_DIALOG` with `sendQuestSelectionDialog(env)`, which sends `SM_DIALOG_WINDOW(npc.getObjectId(), 10, 0)`.
- C# runtime artifact to wire/fix: add live NPC-target finish-dialog handling in `GameServerConnection.HandleDialogSelectAsync`, gated by runtime NPC-start registration and world NPC target resolution.
- Client-visible/state/persistence effect expected: live NPC-target `FINISH_DIALOG` sends `SmDialogWindow(npcObjectId, 10, 0)` without mutating quest state.
...
- Specific behavior/contract to prove: registered NPC-target `FINISH_DIALOG` sends Java-equivalent page `10` and does not mutate quest state.
...
- Wire NPC-target `FINISH_DIALOG` quest selection page `10`.
- Wire broader `AbstractQuestHandler.onDialogEvent` page-only branches only when the exact action/page can be scoped to registered NPC quests and live packet sends.
...
- Total artifacts ported or wired in latest UOW: 1 live NPC-target ask-accept packet path.
