# Phase 6 Session 2142 Handoff - FindGroup Action 4 Direct Packet Trace
...
- Actions `0` and `4` have disabled-boundary-plus-opt-in direct-packet execution trace evidence.
...
This UOW added focused readiness evidence for Java `CM_FIND_GROUP` action `4`:

- Java action `4` calls `FindGroupService.showApplications(player)`.
- Java `showApplications` filters applications by player race and sends `new SM_FIND_GROUP(4, applications)` directly to the triggering player.
- C# disabled boundary composes action `4` as a direct `SmFindGroup` intent.
- A focused test records disabled action `4` acceptance before opt-in registry direct-packet execution to the active player.
- Readiness reports now distinguish action `0`/`4` disabled direct-packet trace evidence from still-missing live `ProcessPacketAsync` direct-packet proof.

## Java Artifacts Touched

- `com.aionemu.gameserver.network.aion.clientpackets.CM_FIND_GROUP`
- `com.aionemu.gameserver.services.findgroup.FindGroupService`
- `com.aionemu.gameserver.services.findgroup.FindGroupService.showApplications`
...
- No live `ProcessPacketAsync` trace proves actions `0` or `4` direct-packet ordering relative to a triggering live `CM_FIND_GROUP` client packet.
...
```text
[Phase 6][UOW-2142] Add find group action four direct trace
```
