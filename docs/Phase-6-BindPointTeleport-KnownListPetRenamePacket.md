# Phase 6 Bind-Point Teleport Known-List Pet Rename Packet

Date: May 27, 2026
Unit of Work: UOW-1299
Status: Complete for the `SM_PET` rename packet shape; Java runtime vector comparison remains unavailable.

## Scope

This unit ports the Java `SM_PET(int petObjectId, String petName)` rename packet shape to C#.

Java payload:

1. `H actionId` where action is `RENAME(10)`;
2. `D petObjectId`;
3. `S petName`.

No live pet rename handler, persistence, validation, or socket send was added.
...
- Added `SmPet(int petObjectId, string petName)`.
- Added a `PetAction.Rename` serializer branch that writes object id then name.
