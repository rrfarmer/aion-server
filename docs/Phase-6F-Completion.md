...
- Extended the `CM_EQUIP_ITEM` path with Java `ItemGroup.getRequiredSkills` parity and silent `Equipment.checkAvailableEquipSkills` rejection before slot mutation.
- Kept `docs/PHASE-6-PROGRESS.md` current through Session 136.
...
- `CM_EQUIP_ITEM` now covers the slot mutation path and Java class/level/max-level/race/gender/AP-rank/required-skill guards, but stigma handling, soul-bind confirmation, identified-item checks, power-shard emotion side effects, quest/summon observers, exact speed/emotion fanout, and full Java stat lifecycle remain pending.
...
1. Finish full `CM_EQUIP_ITEM` parity beyond the current mutation/basic-guard path: stigma handling, soul-bind confirmation, identify checks, power-shard emotion side effects, quest/summon observers, and exact speed/emotion fanout.
