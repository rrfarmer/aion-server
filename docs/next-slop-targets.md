...
### GameServerPacket -> AionServerPacket base-unification FINAL STEP — exact scope of the 11 survivors
All 11 remaining `Sm*` extend the test-only `GameServerPacket` base (gives `WritePayload(PacketBuffer,GameCrypt)` + `SerializeFrame`/`SerializeUnencryptedPayload`); the faithful family extends `AionServerPacket` (`WriteImpl(AionConnection)` + `Write(AionConnection)`/`Encrypt`). Buckets:
- **Abyss points-service big-bang (2):** `SmAbyssRank` (prod: AbyssPointsService/GloryPointsService via `PlayerAbyssRank` projection record), `SmLegionEdit` (prod: AbyssPointsService.Contribution static-factory by-value). Both cross the reworked-projection-vs-live-graph seam; own coordinated tick.
- **Data-model big-bang, dedicated-test-is-sole-byte-oracle (5):** `SmFindGroup`, `SmLegionDominionRank`, `SmLegionHistory`, `SmPet`, `SmPetEmote`. Faithful twins need a live graph the flat-snapshot tests don't build; deleting the tests would orphan the only byte coverage.
- **Crypt-harness seam (2):** `SmKey`, `SmPong` — both test-only, bound to `GameCryptTests`, which exercises `GameCrypt` server framing via the reworked `GameServerPacket.SerializeFrame(GameCrypt)` shortcut. Faithful `SM_KEY`/`SM_PONG` have NO `SerializeFrame`; their only write path is `Write(AionConnection)` + `con.Encrypt`. **Minimal-unblock candidate:** a tiny uninitialized-`AionConnection` harness (no socket; just enough to satisfy `Write`+`Encrypt`/`GetCrypt`) would let GameCryptTests route the faithful packets and retire both — precedent exists (SM_PLAYER_INFO/SM_STATS_INFO went through a Write-seam harness). This is the single highest-leverage base-unification unblock.
...
**RECOMMENDED NEXT:** build the uninitialized-`AionConnection` test harness to unblock `SmKey`/`SmPong` (2-packet retirement, smallest base-unification increment, no production risk). Then the abyss points-service big-bang (retires SmAbyssRank + SmLegionEdit together). The 5 data-model test-only packets and SmGameTime wait on their respective subsystem ports.
