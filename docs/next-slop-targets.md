...
**Residual flight-zone seam (next, unrelated to the runtime island):** `FlightZoneTable`/`FlightZoneSummary` (Dataholders/FlightZoneTable.cs) + `StaticData.FlightZones` are now an UNREAD reworked projection (no production reader after this delete). It is a partial ZoneTemplate/Points slice (FLY/NO_FLY polygons only); the faithful flight/glide path is `ZoneInstance.CanFly/CanGlide` over `ZoneService`/`ZoneInstance`/`WorldMap`. Candidate for a follow-up dead-projection delete once confirmed it has no future-wiring intent.
