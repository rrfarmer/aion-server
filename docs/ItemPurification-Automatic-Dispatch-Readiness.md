...
3. Add quest get/remove callback strategy or a formally documented staged limitation for ItemPurification, next with Java handler/XML quest-start extraction and preserving the no-op dispatcher seam until candidate calculation and quest start-condition evaluation exist.
