...
public sealed class GameServerConnectionGodstoneSocketTests
{
	[Fact]
	public async Task CompleteSocketGodstoneAsync_WritesCleanupSealFlagForRestrictedFullUpdates()
	{
		await using var fixture = await GodstoneSocketFixture.CreateAsync();
		...
		targetUpdate.Godstone = new PlayerGodstone(SourceItemId, ProcCount: 0);
		...
		var plan = new GodstoneSocketPlan(
			GodstoneSocketFailure.None,
			ItemName: "Restricted Godstone Target",
		...
		[
			CreateTemplate(SourceItemId, "Restricted Godstone"),
			CreateTemplate(TargetItemId, "Restricted Godstone Target"),
		]);
		...
		await InvokeCompleteSocketGodstoneAsync(
			fixture.Connection,
			player,
			stoneObjectId: 5001,
			plan,
	...
	private static async Task InvokeCompleteSocketGodstoneAsync(
		...
		ItemRestrictionCleanupTable cleanups)
	{
		var method = typeof(GameServerConnection).GetMethod(
			"CompleteSocketGodstoneAsync",
			BindingFlags.Instance | BindingFlags.NonPublic);
		Assert.NotNull(method);
		var task = Assert.IsAssignableFrom<Task>(method.Invoke(
			connection,
			[player, stoneObjectId, plan, sourceTemplate, itemTemplates, cleanups, CancellationToken.None]));
		await task;
	}
	...
	private const int SourceItemId = 168000001;
	private const int TargetItemId = 100000001;

	private sealed class GodstoneSocketFixture : IAsyncDisposable
	{
		private readonly TcpClient _client;

		private GodstoneSocketFixture(TcpClient client, GameServerConnection connection, List<GameServerPacket> sentPackets)
		{
			_client = client;
		...
		public static async Task<GodstoneSocketFixture> CreateAsync()
		{
			...
			try
			{
				...
				var connection = new GameServerConnection(
					NullLogger.Instance,
					serverClient,
					"godstone-socket-test",
					new GamePacketProcessor<string>((_, _) => Task.CompletedTask),
					sentPacketObserver: sentPackets.Add,
					crypt: crypt);
				return new GodstoneSocketFixture(client, connection, sentPackets);
			}
			finally
			{
				listener.Stop();
