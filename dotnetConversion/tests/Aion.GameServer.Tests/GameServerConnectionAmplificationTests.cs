...
public sealed class GameServerConnectionAmplificationTests
{
	[Fact]
	public async Task CompleteAmplifyItemAsync_WritesCleanupSealFlagForRestrictedMaterialToolAndTargetFullUpdates()
	{
		await using var fixture = await AmplificationFixture.CreateAsync();
		var materialUpdate = CreateItem(objectId: 5001, itemId: MaterialItemId, count: 1);
		var toolUpdate = CreateItem(objectId: 5002, itemId: ToolItemId, count: 1);
		...
		var planInventory = new[] { materialUpdate, toolUpdate, targetUpdate };
		var player = new Player
		{
			ObjectId = 1001,
			InventoryItems =
			[
				CreateItem(objectId: 5001, itemId: MaterialItemId, count: 2),
				CreateItem(objectId: 5002, itemId: ToolItemId, count: 2),
				CreateItem(objectId: 6001, itemId: TargetItemId, count: 1),
			],
		};
		var plan = new AmplificationPlan(
			AmplificationFailure.None,
			ItemName: "Restricted Amplification Target",
			InventoryItems: planInventory,
			TargetItemUpdate: targetUpdate,
			MaterialItemUpdate: materialUpdate,
			DeletedMaterialItemObjectId: null,
			ToolItemUpdate: toolUpdate,
			DeletedToolItemObjectId: null);
		var cleanups = new ItemRestrictionCleanupTable(
		[
			new ItemRestrictionCleanupSummary(MaterialItemId, AccountWarehouse: 0, LegionWarehouse: 1),
			new ItemRestrictionCleanupSummary(ToolItemId, AccountWarehouse: 1, LegionWarehouse: 0),
			new ItemRestrictionCleanupSummary(TargetItemId, AccountWarehouse: 0, LegionWarehouse: 0),
		]);

		await InvokeCompleteAmplifyItemAsync(
			fixture.Connection,
			player,
			plan,
			CreateTemplate(TargetItemId, "Restricted Amplification Target"),
			CreateTemplate(MaterialItemId, "Restricted Amplification Material"),
			CreateTemplate(ToolItemId, "Restricted Amplification Tool"),
			cleanups);
	...
	private static async Task InvokeCompleteAmplifyItemAsync(
		...
		ItemRestrictionCleanupTable cleanups)
	{
		var method = typeof(GameServerConnection).GetMethod(
			"CompleteAmplifyItemAsync",
			BindingFlags.Instance | BindingFlags.NonPublic);
		Assert.NotNull(method);
		var task = Assert.IsAssignableFrom<Task>(method.Invoke(
			connection,
			[player, plan, targetTemplate, materialTemplate, toolTemplate, cleanups]));
		await task;
	}
	...
	private const int MaterialItemId = 166500002;
	private const int ToolItemId = 166500010;
	private const int TargetItemId = 100000001;

	private sealed class AmplificationFixture : IAsyncDisposable
	{
		private readonly TcpClient _client;

		private AmplificationFixture(TcpClient client, GameServerConnection connection, List<GameServerPacket> sentPackets)
		{
			_client = client;
		...
		public static async Task<AmplificationFixture> CreateAsync()
		{
			...
			try
			{
				...
				var connection = new GameServerConnection(
					...
					crypt: crypt);
				return new AmplificationFixture(client, connection, sentPackets);
			}
			finally
			{
				listener.Stop();
