...
public sealed class CmHouseKickSignedPaddingTests
{
	[Fact]
	public void TryCreatePacket_RegistersJavaHouseKickOpcodeAsInGameOnly()
	{
		Assert.IsType<CmHouseKick>(
			GameClientPacketFactory.TryCreatePacket(
				CreateClientPayload(72, buffer =>
				{
					buffer.WriteC(2);
	...
	[Fact]
	public void ReadFrom_HighBitPaddingDoesNotShiftOption()
	{
		...
		Assert.Equal(2, packet.Option);
	}

	private static CmHouseKick CreatePacket()
	{
		return new CmHouseKick(72, new HashSet<GameConnectionState> { GameConnectionState.InGame });
	}
