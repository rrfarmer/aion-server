...
public sealed class VortexLocationServiceTests
{
	...
	[Fact]
	public void DefenderInvitationRegistrationRuntimeAdapter_StoresPendingRequestAndRecordsQuestionWindowIntent()
	{
		var invitationPlanner = new VortexDefenderInvitationPlanService();
		var adapter = new VortexDefenderInvitationRegistrationRuntimeAdapterService();
		var defender = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		var invitation = invitationPlanner.CreatePlan(
			new VortexZonePlayerSnapshot(defender.ObjectId, defender.Race),
			existingDefenderObjectIds: new HashSet<int> { 1001 },
			alliance: VortexDefenderAllianceSnapshot.Open);

		var report = adapter.Register(defender, invitation);

		Assert.Equal(VortexDefenderInvitationRegistrationReportStatus.Registered, report.Status);
		...
		Assert.True(report.ActualPutRequestResult);
		Assert.True(report.RequestStoredByRegistry);
		Assert.True(report.WouldSendQuestionWindow);
		Assert.Equal(0, report.QuestionWindowSenderId);
		Assert.Equal(0, report.QuestionWindowRangeOrCooldownSeconds);
		Assert.True(report.HasPayload);
		Assert.Equal(1, defender.ResponseRequester.Count);
		Assert.True(defender.ResponseRequester.ContainsRequest(SmQuestionWindow.VortexDefenderInvitation));
		...
		Assert.Equal(QuestionResponseRequestKind.VortexDefenderInvitation, dispatch.Request.Kind);
		var payload = Assert.IsType<PendingVortexDefenderInvitationRequest>(dispatch.Request.Payload);
		Assert.Equal(defender.ObjectId, payload.RequesterObjectId);
		Assert.Equal(SmQuestionWindow.VortexDefenderInvitation, payload.QuestionId);
		Assert.Equal(VortexDefenderAllianceSnapshot.Open, payload.DefenderAlliance);
		Assert.Equal([1001], payload.ExistingDefenderObjectIds);
	}
	...
	[Fact]
	public void DefenderInvitationRegistrationRuntimeAdapter_RejectsOccupiedRequestSlotLikeJavaPutIfAbsent()
	{
		var invitationPlanner = new VortexDefenderInvitationPlanService();
		var adapter = new VortexDefenderInvitationRegistrationRuntimeAdapterService();
		var defender = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		...
		Assert.Equal(VortexDefenderInvitationRegistrationReportStatus.RequestRejected, report.Status);
		Assert.True(report.Rejected);
		Assert.False(report.RequestSlotAvailableBeforeRegistration);
		Assert.True(report.AttemptedRequestRegistration);
		Assert.False(report.ActualPutRequestResult);
	...
	[Fact]
	public void DefenderInvitationRegistrationRuntimeAdapter_SkipsGuardedDefenderWithoutRegistryMutation()
	{
		var invitationPlanner = new VortexDefenderInvitationPlanService();
		var adapter = new VortexDefenderInvitationRegistrationRuntimeAdapterService();
		var defender = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		var invitation = invitationPlanner.CreatePlan(
			new VortexZonePlayerSnapshot(defender.ObjectId, defender.Race),
			existingDefenderObjectIds: new HashSet<int> { defender.ObjectId },
			alliance: VortexDefenderAllianceSnapshot.Open);

		var report = adapter.Register(defender, invitation);

		Assert.Equal(VortexDefenderInvitationRegistrationReportStatus.Skipped, report.Status);
	...
	[Fact]
	public void DefenderInvitationResponseRuntimeAdapter_RemovesPlayerRequestAndReturnsConsumptionReport()
	{
		var planner = new VortexDefenderInvitationResponseRuntimeAdapterService();
		...
		var report = planner.HandleResponse(
			responder,
			SmQuestionWindow.VortexDefenderInvitation,
			responseCode: 7);
		...
		Assert.True(report.DispatchPlan?.AcceptancePlan?.WouldRemoveGroup);
		Assert.False(report.DispatchPlan?.AcceptancePlan?.WouldRemoveAlliance);
		Assert.False(report.ShouldRemoveLiveRequest);
		Assert.False(report.ShouldMutateLiveGroup);
		Assert.False(report.ShouldMutateLiveAlliance);
		Assert.False(report.ShouldMutateLiveDefenders);
	}
	...
	[Fact]
	public void DefenderInvitationResponseRuntimeAdapter_ReportsMissingRequestWithoutGameplaySideEffects()
	{
		var planner = new VortexDefenderInvitationResponseRuntimeAdapterService();
		var responder = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		responder.TeamMembership = PlayerTeamMembership.Alliance;
		...
		Assert.False(report.HasVortexPayload);
		Assert.False(report.HasDispatchPlan);
		Assert.Equal(PlayerTeamMembership.Alliance, responder.TeamMembership);
		Assert.False(report.ShouldRemoveLiveRequest);
		Assert.False(report.ShouldMutateLiveGroup);
		Assert.False(report.ShouldMutateLiveAlliance);
		Assert.False(report.ShouldMutateLiveDefenders);
	...
	[Fact]
	public void DefenderInvitationAcceptanceTransitionRuntimeReport_ComposesAcceptedGroupRemovalAndAddPlayerPlan()
	{
		var adapter = new VortexDefenderInvitationResponseRuntimeAdapterService();
		var responder = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		responder.TeamMembership = PlayerTeamMembership.Group;
		...
		var report = adapter.HandleResponseWithAcceptanceTransition(
			responder,
			SmQuestionWindow.VortexDefenderInvitation,
			responseCode: 7,
		...
		Assert.True(report.HasVortexPayload);
		Assert.True(report.HasDispatchPlan);
		Assert.True(report.HasAcceptanceTransitionPlan);
		Assert.True(report.HasAddPlayerPlan);
		Assert.True(report.WouldRemoveGroup);
		Assert.False(report.WouldRemoveAlliance);
		Assert.True(report.WouldCallAddPlayer);
		...
		var updatePlan = Assert.IsType<VortexDefenderUpdateDefendersPlan>(report.UpdateDefendersPlan);
		Assert.Equal(VortexDefenderUpdateDefendersPlanStatus.AcceptancePlanned, updatePlan.Status);
		Assert.Equal(VortexDefenderUpdateDefendersPlanStage.Acceptance, updatePlan.Stage);
		Assert.Equal([1001], updatePlan.ExistingDefenderObjectIds);
		Assert.Equal(VortexDefenderAllianceSnapshot.Missing, updatePlan.DefenderAlliance);
		Assert.Equal(VortexDefenderInvitationAcceptancePlanStatus.AcceptancePlanned, updatePlan.AcceptancePlan?.Status);
		var addPlayerPlan = Assert.IsType<VortexDefenderAddPlayerTransitionPlan>(updatePlan.AddPlayerPlan);
		Assert.Equal(VortexDefenderAddPlayerTransitionPlanStatus.CreateDefenderAlliance, addPlayerPlan.Status);
		Assert.Equal(PlayerAllianceTeamType.AllianceDefence, addPlayerPlan.CreatedAllianceTeamType);
		Assert.Equal([1004, 1001], addPlayerPlan.RemovalPlans.Select(removal => removal.PlayerObjectId).ToArray());
		Assert.False(report.ShouldRemoveLiveRequest);
		Assert.False(report.ShouldMutateLiveGroup);
		Assert.False(report.ShouldMutateLiveAlliance);
		Assert.False(report.ShouldMutateLiveDefenders);
		Assert.Equal(
			"model/gameobjects/player/ResponseRequester.respond -> services/vortex/Invasion.updateDefenders.RequestResponseHandler.acceptRequest",
			report.JavaSource);
	}

	[Fact]
	public void DefenderInvitationAcceptanceTransitionRuntimeReport_FullAllianceBlocksAddAfterRemovalIntent()
	{
		var adapter = new VortexDefenderInvitationResponseRuntimeAdapterService();
		var responder = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		responder.TeamMembership = PlayerTeamMembership.Alliance;
		...
		Assert.True(report.Accepted);
		Assert.True(report.HasAcceptanceTransitionPlan);
		Assert.False(report.HasAddPlayerPlan);
		Assert.False(report.WouldRemoveGroup);
		Assert.True(report.WouldRemoveAlliance);
		Assert.False(report.WouldCallAddPlayer);
		Assert.False(report.WouldPutParticipant);
		Assert.Equal(0, responder.ResponseRequester.Count);
		Assert.Equal(PlayerTeamMembership.Alliance, responder.TeamMembership);
		var updatePlan = Assert.IsType<VortexDefenderUpdateDefendersPlan>(report.UpdateDefendersPlan);
		Assert.Equal(VortexDefenderUpdateDefendersPlanStatus.AcceptanceAllianceFull, updatePlan.Status);
		Assert.Equal(VortexDefenderInvitationAcceptancePlanStatus.DefenderAllianceFull, updatePlan.AcceptancePlan?.Status);
		Assert.Equal(VortexDefenderAllianceSnapshot.Full, updatePlan.DefenderAlliance);
		Assert.False(report.ShouldMutateLiveGroup);
		Assert.False(report.ShouldMutateLiveAlliance);
		Assert.False(report.ShouldMutateLiveDefenders);
	}
	...
	[Fact]
	public void DefenderInvitationAcceptanceTransitionRuntimeReport_DeniedAndMissingResponsesDoNotCreateAcceptancePlan()
	{
		var adapter = new VortexDefenderInvitationResponseRuntimeAdapterService();
		var deniedResponder = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		deniedResponder.TeamMembership = PlayerTeamMembership.Group;
		var missingResponder = CreatePlayer(1006, isOnline: true, worldId: 210060000);
		missingResponder.TeamMembership = PlayerTeamMembership.Alliance;
		var pendingRequest = new PendingVortexDefenderInvitationRequest(
		...
		Assert.True(deniedResponder.ResponseRequester.PutRequest(
			SmQuestionWindow.VortexDefenderInvitation,
			new QuestionResponseRequest(
				deniedResponder.ObjectId,
				QuestionResponseRequestKind.VortexDefenderInvitation,
				pendingRequest)));

		var denied = adapter.HandleResponseWithAcceptanceTransition(
			deniedResponder,
			SmQuestionWindow.VortexDefenderInvitation,
			responseCode: 0);
		var missing = adapter.HandleResponseWithAcceptanceTransition(
			missingResponder,
			SmQuestionWindow.VortexDefenderInvitation,
			responseCode: 1);

		Assert.Equal(VortexDefenderInvitationResponseConsumptionReportStatus.Denied, denied.Status);
		Assert.True(denied.Denied);
		Assert.True(denied.RequestRemovedByRegistry);
		Assert.True(denied.HasDispatchPlan);
		Assert.False(denied.HasAcceptanceTransitionPlan);
		Assert.Null(denied.UpdateDefendersPlan);
		Assert.False(denied.WouldRemoveGroup);
		Assert.False(denied.WouldRemoveAlliance);
		Assert.False(denied.WouldCallAddPlayer);
		Assert.Equal(PlayerTeamMembership.Group, deniedResponder.TeamMembership);
		Assert.Equal(0, deniedResponder.ResponseRequester.Count);

		Assert.Equal(VortexDefenderInvitationResponseConsumptionReportStatus.RequestMissing, missing.Status);
		Assert.False(missing.RequestRemovedByRegistry);
		Assert.False(missing.HasDispatchPlan);
		Assert.False(missing.HasAcceptanceTransitionPlan);
		Assert.Null(missing.UpdateDefendersPlan);
		Assert.Equal(PlayerTeamMembership.Alliance, missingResponder.TeamMembership);
		Assert.False(denied.ShouldMutateLiveGroup);
		Assert.False(denied.ShouldMutateLiveAlliance);
		Assert.False(denied.ShouldMutateLiveDefenders);
		Assert.False(missing.ShouldMutateLiveGroup);
		Assert.False(missing.ShouldMutateLiveAlliance);
		Assert.False(missing.ShouldMutateLiveDefenders);
	}

	[Fact]
	public async Task DefenderAcceptanceParticipantRuntimeReport_RecordsAcceptedParticipantWithoutMutatingRuntime()
	{
		var tempPath = Path.Combine(Path.GetTempPath(), "aion-vortex-defender-accepted-participant-report-" + Guid.NewGuid().ToString("N"));
		Directory.CreateDirectory(tempPath);
		try
		{
			var context = await CreateRuntimeContextAsync(tempPath);
			...
			var adapter = new VortexDefenderInvitationResponseRuntimeAdapterService();
			var reporter = new VortexDefenderAcceptanceParticipantRuntimeReportService();
			...
			var transition = adapter.HandleResponseWithAcceptanceTransition(
				responder,
				SmQuestionWindow.VortexDefenderInvitation,
				responseCode: 7,
				existingDefenders: [existingDefender]);
			var beforeSnapshot = Assert.IsType<VortexInvasionSnapshot>(runtime.GetSnapshot(location.Id));
			var report = reporter.CreateReport(
				location.Id,
				transition);
			var afterSnapshot = Assert.IsType<VortexInvasionSnapshot>(runtime.GetSnapshot(location.Id));

			Assert.Equal(VortexDefenderAcceptanceParticipantRuntimeReportStatus.ParticipantWouldBeRecorded, report.Status);
			...
			Assert.True(report.HasAddPlayerPlan);
			Assert.True(report.WouldPutParticipant);
			Assert.True(report.WouldCreateDefenderAlliance);
			Assert.False(report.WouldAddToExistingAlliance);
			Assert.False(report.WouldWarn);
			Assert.Same(transition, report.TransitionReport);
			Assert.Same(transition.UpdateDefendersPlan?.AddPlayerPlan, report.AddPlayerPlan);
			Assert.Empty(beforeSnapshot.DefenderObjectIds);
			Assert.Empty(afterSnapshot.DefenderObjectIds);
			Assert.Equal(PlayerTeamMembership.Group, responder.TeamMembership);
			Assert.False(report.ShouldMutateLiveGroup);
			Assert.False(report.ShouldMutateLiveAlliance);
			Assert.False(report.ShouldMutateLiveDefenders);
			Assert.False(report.ShouldSendLivePacket);
			Assert.Equal(
				"services/vortex/Invasion.addPlayer(player, false) -> defenders.put(player.getObjectId(), player)",
				report.JavaSource);
		}
		finally
		{
	...
	[Fact]
	public void DefenderAcceptanceParticipantRuntimeReport_PreservesNoMutationForDeniedMissingFullAllianceAndWarn()
	{
		var adapter = new VortexDefenderInvitationResponseRuntimeAdapterService();
		var reporter = new VortexDefenderAcceptanceParticipantRuntimeReportService();
		var deniedResponder = CreatePlayer(1004, isOnline: true, worldId: 210060000);
		var missingResponder = CreatePlayer(1006, isOnline: true, worldId: 210060000);
		var fullResponder = CreatePlayer(1007, isOnline: true, worldId: 210060000);
		var warnResponder = CreatePlayer(1008, isOnline: true, worldId: 210060000);
		var deniedRequest = new PendingVortexDefenderInvitationRequest(
			RequesterObjectId: deniedResponder.ObjectId,
			QuestionId: SmQuestionWindow.VortexDefenderInvitation,
			DefenderAlliance: VortexDefenderAllianceSnapshot.Open,
			ExistingDefenderObjectIds: [1001]);
		var fullRequest = new PendingVortexDefenderInvitationRequest(
			RequesterObjectId: fullResponder.ObjectId,
			QuestionId: SmQuestionWindow.VortexDefenderInvitation,
			DefenderAlliance: VortexDefenderAllianceSnapshot.Full,
			ExistingDefenderObjectIds: [1001]);
		var warnRequest = new PendingVortexDefenderInvitationRequest(
			RequesterObjectId: warnResponder.ObjectId,
			QuestionId: SmQuestionWindow.VortexDefenderInvitation,
			DefenderAlliance: VortexDefenderAllianceSnapshot.Missing,
			ExistingDefenderObjectIds: [1001, 1002]);
		Assert.True(deniedResponder.ResponseRequester.PutRequest(
			SmQuestionWindow.VortexDefenderInvitation,
			new QuestionResponseRequest(deniedResponder.ObjectId, QuestionResponseRequestKind.VortexDefenderInvitation, deniedRequest)));
		Assert.True(fullResponder.ResponseRequester.PutRequest(
			SmQuestionWindow.VortexDefenderInvitation,
			new QuestionResponseRequest(fullResponder.ObjectId, QuestionResponseRequestKind.VortexDefenderInvitation, fullRequest)));
		Assert.True(warnResponder.ResponseRequester.PutRequest(
			SmQuestionWindow.VortexDefenderInvitation,
			new QuestionResponseRequest(warnResponder.ObjectId, QuestionResponseRequestKind.VortexDefenderInvitation, warnRequest)));

		var denied = reporter.CreateReport(
			locationId: 0,
			adapter.HandleResponseWithAcceptanceTransition(
				deniedResponder,
				SmQuestionWindow.VortexDefenderInvitation,
				responseCode: 0),
			currentDefenderObjectIds: [1001]);
		var missing = reporter.CreateReport(
			locationId: 0,
			adapter.HandleResponseWithAcceptanceTransition(
				missingResponder,
				SmQuestionWindow.VortexDefenderInvitation,
				responseCode: 1),
			currentDefenderObjectIds: [1001]);
		var full = reporter.CreateReport(
			locationId: 0,
			adapter.HandleResponseWithAcceptanceTransition(
				fullResponder,
				SmQuestionWindow.VortexDefenderInvitation,
				...
				defenderAlliance: VortexDefenderAllianceSnapshot.Full),
			currentDefenderObjectIds: [1001]);
		var warn = reporter.CreateReport(
			locationId: 0,
			adapter.HandleResponseWithAcceptanceTransition(
				warnResponder,
				SmQuestionWindow.VortexDefenderInvitation,
			...
			currentDefenderObjectIds: [1001, 1002]);

		Assert.Equal(VortexDefenderAcceptanceParticipantRuntimeReportStatus.NoParticipantMutation, denied.Status);
		Assert.Equal(VortexDefenderAcceptanceParticipantRuntimeReportStatus.NoParticipantMutation, missing.Status);
		Assert.Equal(VortexDefenderAcceptanceParticipantRuntimeReportStatus.NoParticipantMutation, full.Status);
		Assert.Equal(VortexDefenderAcceptanceParticipantRuntimeReportStatus.WarningNoParticipantPut, warn.Status);
		...
		Assert.Equal([1001], full.DefenderObjectIdsBefore);
		Assert.Equal([1001], full.DefenderObjectIdsAfter);
		Assert.Equal([1001, 1002], warn.DefenderObjectIdsBefore);
		Assert.Equal([1001, 1002], warn.DefenderObjectIdsAfter);
		Assert.False(denied.WouldRecordParticipant);
		Assert.False(missing.WouldRecordParticipant);
		Assert.False(full.WouldRecordParticipant);
		Assert.False(warn.WouldRecordParticipant);
		Assert.False(denied.HasAddPlayerPlan);
		Assert.False(missing.HasAddPlayerPlan);
		Assert.False(full.HasAddPlayerPlan);
		Assert.True(warn.HasAddPlayerPlan);
		Assert.True(warn.WouldWarn);
		Assert.False(warn.WouldPutParticipant);
		foreach (var report in new[] { denied, missing, full, warn })
		{
			Assert.False(report.ShouldMutateLiveGroup);
			Assert.False(report.ShouldMutateLiveAlliance);
			Assert.False(report.ShouldMutateLiveDefenders);
