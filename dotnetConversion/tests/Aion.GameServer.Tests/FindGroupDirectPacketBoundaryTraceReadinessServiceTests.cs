...
public sealed class FindGroupDirectPacketBoundaryTraceReadinessServiceTests
{
	...
	public void CreateReport_RecordsDisabledBoundaryActionZeroAndOptInTraceEvidence()
	{
		...
		Assert.Contains(
			report.Evidence,
			evidence => evidence.Kind == FindGroupDirectPacketBoundaryTraceEvidenceKind.CSharpLiveBoundaryTraceContract
				&& evidence.Status == FindGroupDirectPacketBoundaryTraceEvidenceStatus.EvidenceAvailable
		...
		Assert.Contains(
			report.Evidence,
			evidence => evidence.Kind == FindGroupDirectPacketBoundaryTraceEvidenceKind.CSharpShowListTraceSchema
				&& evidence.Status == FindGroupDirectPacketBoundaryTraceEvidenceStatus.EvidenceAvailable
				&& evidence.Detail.Contains("FindGroupDirectPacketShowListBoundaryTraceSchemaService", StringComparison.Ordinal)
		...
		Assert.Contains(
			report.Evidence,
			evidence => evidence.Kind == FindGroupDirectPacketBoundaryTraceEvidenceKind.CSharpMutationPostTraceExportProjection
				&& evidence.Status == FindGroupDirectPacketBoundaryTraceEvidenceStatus.EvidenceAvailable
				&& evidence.Detail.Contains("schema version 1 C# exports", StringComparison.Ordinal)
				&& evidence.Detail.Contains("disabled action 2 and 6 boundary plans", StringComparison.Ordinal)
				&& evidence.Detail.Contains("without invoking the executor", StringComparison.Ordinal));
	}
	...
	public void CreateReport_RecordsNextRequiredLiveEvidence()
	{
		...
		Assert.Contains(
			report.NextRequiredEvidence,
			item => item.Contains("mutation-post export projection helper", StringComparison.Ordinal));
		Assert.Contains(
			report.NextRequiredEvidence,
