...
public class GamePacketTests
{
	...
	public void SmSystemMessage_WritesDialogTooFarMessages()
	{
		...
		AssertSystemMessage(SmSystemMessage.BuySellHeDoesNotSellItem("Merchant"), 1300336, "Merchant");
	...
	public void CharacterSelectionServerPackets_WriteJavaShapedPayloads()
	{
		...
		var statUpdateMpPayload = SerializeUnencryptedPayload(new SmStatUpdateMp(currentMp: 222, maxMp: 333));
		using var statUpdateMpReader = new PacketBuffer(statUpdateMpPayload);
		Assert.Equal(222, statUpdateMpReader.ReadD());
		Assert.Equal(333, statUpdateMpReader.ReadD());
		Assert.Equal(0, statUpdateMpReader.Remaining);

		var statUpdateDpPayload = SerializeUnencryptedPayload(new SmStatUpdateDp(currentDp: 3210));
		using var statUpdateDpReader = new PacketBuffer(statUpdateDpPayload);
		Assert.Equal(3210, statUpdateDpReader.ReadH());
		Assert.Equal(0, statUpdateDpReader.Remaining);

		var dpInfoPayload = SerializeUnencryptedPayload(new SmDpInfo(playerObjectId: 1003, currentDp: 4000));
		using var dpInfoReader = new PacketBuffer(dpInfoPayload);
		Assert.Equal(1003, dpInfoReader.ReadD());
		Assert.Equal(4000, dpInfoReader.ReadH());
		Assert.Equal(0, dpInfoReader.Remaining);
		...
		AssertSystemMessage(SmSystemMessage.MsgNotEnoughAbyssPoints(), 1300927);
		...
		AssertSystemMessage(SmSystemMessage.GloryPointGain(45), 1402081, "45");
		AssertSystemMessage(SmSystemMessage.GloryPointLose(12), 1402219, "12");
		...
		AssertSystemMessage(SmSystemMessage.WarehouseTooManyItemsInventory(), 1300416);
		AssertSystemMessage(SmSystemMessage.CantUseHouseObjectItemCheck("item"), 1401199, "item");
		...
		AssertSystemMessage(SmSystemMessage.HousingCannotUseFlowerpotCooltime(), 1401280);
		...
		AssertSystemMessage(SmSystemMessage.CantUseHouseObjectItemEquip("item"), 1401294, "item");
		AssertSystemMessage(SmSystemMessage.HousingObjectAchieveUseCount(), 1401295);
		AssertSystemMessage(SmSystemMessage.HousingObjectAllCantUse(), 1401296);
		...
		AssertSystemMessage(SmSystemMessage.HousingFlowerpotGoal("object"), 1401333, "object");
		...
		Assert.Equal(2878.98f, brokerNpcReader.ReadF());
		Assert.Equal(532.835f, brokerNpcReader.ReadF());
		Assert.Equal(5001, brokerNpcReader.ReadD());
		...
		Assert.Equal(0, brokerNpcReader.ReadH());
		Assert.Equal(0, (int)brokerNpcReader.ReadC());
		Assert.Equal(0, brokerNpcReader.ReadD());
		Assert.Equal(0, brokerNpcReader.ReadD());
		Assert.Equal(string.Empty, brokerNpcReader.ReadS());
		Assert.Equal(100, (int)brokerNpcReader.ReadC());
		Assert.Equal(5000, brokerNpcReader.ReadD());
		Assert.Equal(55, (int)brokerNpcReader.ReadC());
		Assert.Equal(0, brokerNpcReader.ReadD());
		Assert.Equal(0.6f, brokerNpcReader.ReadF());
		Assert.Equal(1.98f, brokerNpcReader.ReadF());
		Assert.Equal(4.5f, brokerNpcReader.ReadF());
		Assert.Equal(2000, brokerNpcReader.ReadH());
		Assert.Equal(2000, brokerNpcReader.ReadH());
		Assert.Equal(0, (int)brokerNpcReader.ReadC());
		Assert.Equal(1887.75f, brokerNpcReader.ReadF());
		Assert.Equal(2878.98f, brokerNpcReader.ReadF());
	...
	[Fact]
	public void SmInventoryAddItem_CraftedItemWritesCraftedAddTypeAndCleanupSealFlagLikeJava()
	{
		var payload = SerializeUnencryptedPayload(
			SmInventoryAddItem.CreateCraftedItem(
				new InventoryItem
				{
					ObjectId = 91,
					ItemId = 188053996,
		...
		using var reader = new PacketBuffer(payload);
		Assert.Equal(SmInventoryAddItem.CraftedItem, reader.ReadH());
		Assert.Equal(1, reader.ReadH());
		var restrictedItem = ReadInventoryItemWithBlob(reader);
		Assert.Equal((91, 188053996, 11, 0), (restrictedItem.ObjectId, restrictedItem.ItemId, restrictedItem.EquipmentSlot, restrictedItem.IsCloth));
		Assert.Equal(0, reader.Remaining);
	...
	[Fact]
	public void SmInventoryAddItem_BrokerBuyWritesCleanupSealFlagInItemBlobLikeJava()
	{
		var payload = SerializeUnencryptedPayload(
		...
		Assert.Equal(1, reader.ReadH());
		var restrictedItem = ReadInventoryItemWithBlob(reader);
		Assert.Equal((89, 188053996, 9, 0), (restrictedItem.ObjectId, restrictedItem.ItemId, restrictedItem.EquipmentSlot, restrictedItem.IsCloth));
		Assert.Equal(0, reader.Remaining);
	...
	public void ClientPacketFactory_ParsesItemPurificationPacket()
	{
		...
		Assert.Equal(100201416, packet.ResultItemId);
		Assert.Equal([9101, 9102, 9103, 9104, 9105], packet.RequiredMaterialObjectIds);
		Assert.Null(GameClientPacketFactory.TryCreatePacket(CreateClientPayload(247, _ => { }), GameConnectionState.Authed));
	}
	...
	public void ClientPacketFactory_ParsesStopTrainingPacket()
	{
		...
		Assert.Null(GameClientPacketFactory.TryCreatePacket(CreateClientPayload(84, _ => { }), GameConnectionState.Authed));
	}
	...
	[Fact]
	public void ClientPacketFactory_ParsesAbyssRankingLegionsPacket()
	{
		var rankingLegions = Assert.IsType<CmAbyssRankingLegions>(
			GameClientPacketFactory.TryCreatePacket(CreateClientPayload(118, buffer => buffer.WriteC(1)), GameConnectionState.InGame)
		);

		Assert.Equal(1, rankingLegions.RaceId);
		Assert.Null(GameClientPacketFactory.TryCreatePacket(CreateClientPayload(118, buffer => buffer.WriteC(0)), GameConnectionState.Authed));
	}
