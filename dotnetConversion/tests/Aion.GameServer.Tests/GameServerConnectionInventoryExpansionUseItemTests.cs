...
public sealed class GameServerConnectionInventoryExpansionUseItemTests
{
	...
	public async Task ProcessPacketAsync_CubeSourceSplitsItemToLegionWarehouseOwnerLikeJava()
	{
		...
		Assert.Equal(1001, savedSplit.SourceItem.OwnerId);
		Assert.Equal(3, savedSplit.SourceItem.Count);
		Assert.Equal(1, savedSplit.NewItem.ObjectId);
		Assert.Equal(88, savedSplit.NewItem.OwnerId);
	...
	[Fact]
	public async Task ProcessPacketAsync_CubeSourceSplitToFullLegionWarehouseSendsJavaFullMessageWithoutMutation()
	{
		var repository = new EmptyPlayerEnterWorldRepository();
		await using var fixture = await InventoryExpansionUseItemFixture.CreateAsync(repository, idFactory: new IDFactory([5001]));
		var player = CreatePlayer(itemId: 200, count: 5, accountId: 77);
		...
		player.LegionVolunteerPermission = 0x1000;
		player.InventoryItems = player.InventoryItems
			.Concat(CreateStorageFillerItems(location: 3, count: InventoryCapacity.GetLegionWarehouseLimit(player), ownerId: player.LegionId))
			.ToArray();
		SetActivePlayerForPacketDispatch(fixture.Connection, player);
	...
	[Fact]
	public async Task ProcessPacketAsync_CubeSourceMoveToFullLegionWarehouseSendsJavaFullMessageAndUnlocksSource()
	{
		var repository = new EmptyPlayerEnterWorldRepository();
		await using var fixture = await InventoryExpansionUseItemFixture.CreateAsync(repository, idFactory: new IDFactory([5001]));
		var player = CreatePlayer(itemId: 200, count: 7);
		var sourceItem = Assert.Single(player.InventoryItems);
		sourceItem.Slot = 12;
		player.LegionId = 77;
		player.LegionLevel = 1;
	...
	[Fact]
	public async Task ProcessPacketAsync_LegionWarehouseKinahDepositMutatesLikeJava()
	{
		var repository = new EmptyPlayerEnterWorldRepository();
		await using var fixture = await InventoryExpansionUseItemFixture.CreateAsync(repository, idFactory: new IDFactory([5001]));
		var player = CreatePlayer(itemId: 182400001, count: 5000);
		...
		Assert.Collection(
			...
			item =>
			{
				...
				Assert.Equal(1001, item.OwnerId);
				Assert.Equal(0, item.Location);
				Assert.Equal(4000, item.Count);
			});
		Assert.Equal(1, repository.SaveItemMergeMutationCalls);
		var savedMerge = Assert.NotNull(repository.SavedItemMergeMutation);
		Assert.Equal(1001, savedMerge.PlayerObjectId);
		Assert.Equal(5001, savedMerge.SourceItem.ObjectId);
		Assert.Equal(4000, savedMerge.SourceItem.Count);
		Assert.Equal(1001, savedMerge.SourceItem.OwnerId);
		Assert.Equal(1, savedMerge.TargetItem.ObjectId);
		Assert.Equal(1000, savedMerge.TargetItem.Count);
		Assert.Equal(77, savedMerge.TargetItem.OwnerId);
