...
public sealed class FindGroupMutationPostProjectedRowComparisonRuntimeEvidenceChecklistServiceTests
{
	...
	public void Create_CSharpBoundaryAndRegistryRowsStayNonLive()
	{
		...
		Assert.Contains(checklist.Rows, row =>
			...
			&& row.ExistingProvider.Contains("FindGroupMutationPostProjectedValueMaterializationBlockerReportService", StringComparison.Ordinal)
			&& row.ExistingProvider.Contains("FindGroupMutationPostProjectedValueResultEmissionBlockerReportService", StringComparison.Ordinal)
			&& row.ExistingProvider.Contains("FindGroupMutationPostProjectedValueExecutorEvidenceBridgeService", StringComparison.Ordinal)
			...
			&& row.RequiredNextEvidence.Contains("projected-value materialization blocker report", StringComparison.Ordinal)
			&& row.RequiredNextEvidence.Contains("projected-value result-emission blocker report", StringComparison.Ordinal)
			&& row.RequiredNextEvidence.Contains("projected-value executor evidence bridge", StringComparison.Ordinal)
