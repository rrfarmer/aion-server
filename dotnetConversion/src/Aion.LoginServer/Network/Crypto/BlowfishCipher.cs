...
public sealed class BlowfishCipher
{
	...
	public void UpdateKey(byte[] blowfishKey)
	{
		...
		Array.Copy(SBoxInit0, 0, _sBoxes[0], 0, 256);
		Array.Copy(SBoxInit1, 0, _sBoxes[1], 0, 256);
		Array.Copy(SBoxInit2, 0, _sBoxes[2], 0, 256);
		Array.Copy(SBoxInit3, 0, _sBoxes[3], 0, 256);
		InitArrays();
	}
	...
	private void InitArrays()
	{
		unchecked
		{
			...
			for (var i = 0; i < 18; i++)
			{
				...
				for (var j = 0; j < 4; j++)
				{
					data = (data << 8) | (_blowfishKey[keyIndex++] & 0xFF);
					if (keyIndex >= _blowfishKey.Length)
						keyIndex = 0;
				}
				_pArray[i] = PArrayInit[i] ^ data;
			}
			...
			{
				Cipher(b);
				_pArray[i] = ByteArrayToInteger(b, 0);
				_pArray[i + 1] = ByteArrayToInteger(b, 4);
			}

			for (var i = 0; i < 4; i++)
				InitSBox(b, _sBoxes[i]);
		}
	}
