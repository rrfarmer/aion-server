...
public static class NearbyQuestCandidateProjectionService
{
	...
	{
		// Java parity breadcrumb: WorldMapInstance.addObject(Npc) reads QuestEngine.getQuestNpc(npcId).getOnQuestStart().
		...
		return new NearbyQuestCandidateProjectionResult(
			inspectedNpcIds,
			matchedNpcIds,
			projectedQuestIds,
			newlyRegisteredQuestIds,
			currentQuestIds);
	}
}
