...
public static class QuestAbandonService
{
		...
		Func<int, bool>? hasQuestHandler = null)
	{
		...
		if (questState.CompleteCount > 0)
		{
			...
			var workItemDeletions = QuestWorkItemRemovalService.RemoveQuestWorkItems(player, template, reset);
		...
		var deletions = QuestWorkItemRemovalService.RemoveQuestWorkItems(player, template, questState);
