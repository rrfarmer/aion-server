		...
		return new FindGroupConnectionBoundaryReadinessAggregate(
			...
			[
				...
				new FindGroupConnectionBoundaryComponentReadiness(
					...
					"FindGroupSideEffectDispatchAuditService can audit direct sendPacket and broadcastToWorld intents; FindGroupSideEffectDispatchExecutorService can execute them through IGameClientConnectionRegistry only when explicitly invoked and records direct-before-broadcast execution order; FindGroupConnectionBoundarySideEffectCompositionEvidenceService can compose parsed CmFindGroup plans, including action 0/4 show-list direct packets, action 2/3/6/7 recruitment/application mutation evidence, action 8/9/17 instance-group mutation packets, action 10/13 instance-group show direct packets, action 11/12 instance-application direct/invite intents, and action 1/5 world-broadcast intents, with opt-in executor results.",
					FindGroupConnectionBoundaryComponentStatus.EvidenceAvailable,
				...
				new FindGroupConnectionBoundaryComponentReadiness(
					"Lifecycle observers",
					"Logout, joined-team, and disband cleanup have production singleton graph evidence, and FindGroupRecruitmentPlanService uses ConcurrentDictionary state stores matching Java ConcurrentHashMap shape.",
				...
				new FindGroupConnectionBoundaryComponentReadiness(
					...
					"FindGroupLifecycleSingletonWiringReadinessService enumerates Java singleton call sites for CM_FIND_GROUP, logout cleanup, group/alliance joined-team cleanup, and group/alliance disband recruitment removal; current C# lifecycle evidence remains mixed production graph and deferred boundary evidence.",
					FindGroupConnectionBoundaryComponentStatus.PartialEvidence,
