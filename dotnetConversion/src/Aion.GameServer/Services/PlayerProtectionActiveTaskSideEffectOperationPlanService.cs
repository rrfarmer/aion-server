...
public static class PlayerProtectionActiveTaskSideEffectOperationPlanService
{
	...
	{
		var rows = request.Action == PlayerProtectionActiveTaskAdapterAction.Start
		...
		return new PlayerProtectionActiveTaskSideEffectOperationPlan(
			...
			rowList,
			SchedulesDelayedStop: rowList.Any(row => row.Operation == PlayerProtectionActiveTaskSideEffectOperation.ScheduleDelayedStopTask && row.JavaCallReached),
			CancelsExistingStopTask: rowList.Any(row => row.Operation == PlayerProtectionActiveTaskSideEffectOperation.CancelProtectionTask && row.JavaCallReached),
			CancelsKnownCreatureCasts: rowList.Any(row => row.Operation == PlayerProtectionActiveTaskSideEffectOperation.CancelCastOnKnownCreatures && row.JavaCallReached),
			ClearsKnownPlayerTargets: rowList.Any(row => row.Operation == PlayerProtectionActiveTaskSideEffectOperation.RemoveTargetFromKnownPlayers && row.JavaCallReached),
			...
			SkipsAiMoveNotificationForFlightPath: rowList.Any(row => row.Operation == PlayerProtectionActiveTaskSideEffectOperation.SkipNotifyAiOnMoveForFlightPath && row.JavaCallReached),
			HasLiveVisualMutationOnly: adapterResult.MutatedVisualState && rowList.All(row => !row.IsLive || row.Operation is PlayerProtectionActiveTaskSideEffectOperation.SetBlinkingVisualState or PlayerProtectionActiveTaskSideEffectOperation.UnsetBlinkingVisualState),
			"PlayerController.startProtectionActiveTask / stopProtectionActiveTask ordered side-effect plan",
			IsLive: adapterResult.IsLive);
	}

	private static IEnumerable<PlayerProtectionActiveTaskSideEffectOperationRow> CreateStartRows(
		PlayerProtectionActiveTaskAdapterResult adapterResult)
	{
		yield return Row(
			0,
			...
			adapterResult.Plan.WasProtectionActive ? "Already protected; Java exits without side effects." : "Protection inactive; Java enters side-effect branch.");
		...
		PlayerProtectionActiveTaskAdapterResult adapterResult)
	{
		...
		var usingFlightPath = player.IsUsingFlightPath(PlayerFlightPathType.FlightTransporter)
			|| player.IsUsingFlightPath(PlayerFlightPathType.Windstream);
		yield return Row(
			4,
			usingFlightPath
