...
public sealed record NpcFactionPersistenceStateEntry(
...
public static class NpcFactionPersistencePlanService
{
	...
	public static NpcFactionPersistencePlan CreatePlan(
		IEnumerable<NpcFactionPersistenceStateEntry> factionStates)
	{
		...
		foreach (var entry in factionStates)
		{
			switch (entry.PersistenceState)
			{
				case NpcFactionPersistenceState.New:
					descriptors.Add(new NpcFactionPersistenceOperationDescriptor(
						order++,
