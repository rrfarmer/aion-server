...
public sealed class PlayerKnownListPetSpawnSnapshotProviderService
{
	...
	{
		// Java parity: SM_PET(Pet) reads live Pet, PetCommonData,
		...
		if (input.PetObjectId is null)
			return Blocked(
				PlayerKnownListPetSpawnSnapshotProviderStatus.MissingActivePet,
				input,
				javaSource,
				"No active pet object id was supplied; Java VisibleObjectSpawner.spawnPet would not produce SM_PET(Pet)."
			);
		if (string.IsNullOrEmpty(input.PetName))
			return Blocked(
				PlayerKnownListPetSpawnSnapshotProviderStatus.MissingPetName,
				input,
				javaSource,
				"Java SM_PET(Pet) writes pet.getName(), sourced from PetCommonData.getName()."
			);
		if (input.PetTemplateId is null)
			return Blocked(
				PlayerKnownListPetSpawnSnapshotProviderStatus.MissingPetTemplate,
				input,
				javaSource,
				"Java SM_PET(Pet) writes pet.getObjectTemplate().getTemplateId()."
			);
		if (input.X is null || input.Y is null || input.Z is null)
			return Blocked(
				PlayerKnownListPetSpawnSnapshotProviderStatus.MissingPetPosition,
				input,
				javaSource,
				"Java SM_PET(Pet) writes pet.getPosition().getX/Y/Z()."
			);
		if (input.TargetX is null || input.TargetY is null || input.TargetZ is null)
			return Blocked(
				PlayerKnownListPetSpawnSnapshotProviderStatus.MissingPetMoveTarget,
				input,
				javaSource,
				"Java SM_PET(Pet) writes pet.getMoveController().getTargetX2/Y2/Z2()."
			);
		...
		if (input.PetMasterObjectId is null)
			return Blocked(
				PlayerKnownListPetSpawnSnapshotProviderStatus.MissingPetMaster,
				input,
				javaSource,
				"Java SM_PET(Pet) writes pet.getMaster().getObjectId()."
			);
		if (input.CommonDataDecoration is null)
			return Blocked(
				PlayerKnownListPetSpawnSnapshotProviderStatus.MissingPetCommonData,
				input,
				javaSource,
				"Java SM_PET(Pet) writes pet.getCommonData().getDecoration()."
			);
	...
	private static PlayerKnownListPetSpawnSnapshotProviderResult Blocked(
		...
		new(status, Snapshot: null, input.MasterIsInFlyingState, CanCreateFlyStartEmote: false, IsLive: false, IsJavaPetParity: false, javaSource, notes);
}
