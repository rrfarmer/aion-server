...
public class PvpService
{
    ...
    public void DoReward(Player victim, float apWinMulti)
    {
        ...
        if (mostDamage == null || !(mostDamage.GetAttacker() is Player winner))
        {
            ...
            TemporaryPlayerTeam<TeamMember<Player>> team = victim.GetCurrentTeam();
            if (team != null)
        ...
        foreach (DamageInfo<AionObject> damageInfo in damageList.ToTeamDamages().GetCreatureOrTeamDamages())
        {
            ...
            else if (attacker is TemporaryPlayerTeam<TeamMember<Player>> team && team.GetLeaderObject().GetRace() != victim.GetRace())
    ...
    {
        TemporaryPlayerTeam<TeamMember<Player>> group = winner.GetCurrentGroup();
        List<Player> killers;
        if (group == null)
