		...
		FindGroupMutationPostProjectedRowComparisonLiveInputRequirement requirement)
	{
		return requirement switch
		{
			...
			FindGroupMutationPostProjectedRowComparisonLiveInputRequirement.RowIdentityMatching => new RuntimeEvidenceMapping(
				HasExistingProvider: true,
				"FindGroupMutationPostCSharpAcceptedBoundaryRowHandoffReportService, FindGroupMutationPostJavaCSharpRowPairingReadinessReportService, FindGroupMutationPostProjectedRowComparisonDryRunContractService, FindGroupMutationPostProjectedRowComparisonExecutorSkeletonService",
				"Match action, mutationKind, activePlayerObjectId, and mutatedEntryObjectId across runtime Java and C# rows after the explicit-root Java summary and accepted-boundary-row handoff prove action/mutation pairing readiness through the Java/C# row-pairing readiness report.",
				"Accepted-boundary-row handoff, Java/C# row pairing readiness, dry-run paired readiness, and executor skeleton rows remain metadata until runtime row keys are inspected and values are projected."),
