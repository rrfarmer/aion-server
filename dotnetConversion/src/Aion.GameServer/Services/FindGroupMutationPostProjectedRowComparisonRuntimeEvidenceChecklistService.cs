		...
		FindGroupMutationPostProjectedRowComparisonLiveInputRequirement requirement)
	{
		return requirement switch
		{
			...
			FindGroupMutationPostProjectedRowComparisonLiveInputRequirement.ResultEmission => new RuntimeEvidenceMapping(
				...
				"Emit real Matched, MissingJavaRow, MissingCSharpRow, FieldMismatch, and IgnoredRuntimeContext rows from projected comparison using the value-reader result schema and comparator preflight.",
				"Result skeletons, value-reader result schema, and comparator preflight describe output and executor stages but cannot materialize them."),
