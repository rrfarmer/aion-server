	...
	public static FindGroupLiveDispatchReadinessReport CreateReport()
	{
		...
		return new FindGroupLiveDispatchReadinessReport(
			...
			[
				...
				"FindGroupConnectionBoundarySideEffectCompositionEvidenceService can compose parsed CmFindGroup planner output, including action 0/4 show-list direct packets, action 10/13 instance-group show direct packets, and action 1/5 world-broadcast intents, with opt-in executor results without wiring GameServerConnection.",
			],
