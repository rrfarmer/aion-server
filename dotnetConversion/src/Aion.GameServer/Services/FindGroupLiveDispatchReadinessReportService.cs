	...
	public static FindGroupLiveDispatchReadinessReport CreateReport()
	{
		...
		return new FindGroupLiveDispatchReadinessReport(
			...
			[
				...
				"FindGroupDirectPacketBoundaryTraceReadinessService records action 0 disabled-boundary acceptance before opt-in registry execution of the direct SmFindGroup packet while keeping live ProcessPacketAsync ordering blocked.",
				...
				"FindGroupWorldBroadcastFanoutReadinessService separates Java PacketSendUtility.broadcastToWorld race-filter review and C# opt-in registry fanout plus disabled action 1 boundary fanout trace evidence from the still-missing live CM_FIND_GROUP actions 1/5 fanout proof.",
