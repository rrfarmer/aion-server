...
public sealed record QuestPersistenceStateEntry(
	PlayerQuestState QuestState,
	QuestPersistenceState PersistenceState);
...
public sealed record QuestPersistencePlan(
	QuestPersistencePlanStatus Status,
	IReadOnlyList<QuestPersistenceOperationDescriptor> Descriptors)
{
...
public static class QuestPersistencePlanService
{
	...
	public static QuestPersistencePlan CreatePlan(
		IEnumerable<QuestPersistenceStateEntry> questStates,
		IEnumerable<int>? deletedQuestIds = null)
	{
		...
		foreach (var entry in entries
			.Where(entry => entry.PersistenceState == QuestPersistenceState.Deleted)
			.OrderBy(entry => entry.QuestState.QuestId))
		{
			descriptors.Add(new QuestPersistenceOperationDescriptor(
				order++,
		...
		foreach (var entry in entries
			.Where(entry => entry.PersistenceState == QuestPersistenceState.New)
			.OrderBy(entry => entry.QuestState.QuestId))
		{
			descriptors.Add(new QuestPersistenceOperationDescriptor(
				order++,
		...
		foreach (var entry in entries
			.Where(entry => entry.PersistenceState == QuestPersistenceState.UpdateRequired)
			.OrderBy(entry => entry.QuestState.QuestId))
		{
			descriptors.Add(new QuestPersistenceOperationDescriptor(
				order++,
		...
		return new QuestPersistencePlan(
			descriptors.Count == 0 ? QuestPersistencePlanStatus.NoChanges : QuestPersistencePlanStatus.Ready,
			descriptors);
	}
}
