...
{
	// Java parity: model/templates/item/actions/DecomposeAction.act controls the delayed normal
	// decompose flow, including animation, source consume, success message, reward add, and final animation order.
	public const string JavaDecomposeActionSource = "game-server/src/com/aionemu/gameserver/model/templates/item/actions/DecomposeAction.java";

	public const string CSharpHandlerSource = "dotnetConversion/src/Aion.GameServer/Network/Aion/GameServerConnection.cs#CompleteDecomposeUseItemAsync";

	public static DecomposeRewardOrderingPlan CreatePlan()
	{
		// Java parity: this plan captures the visible Java packet and mutation order so the staged C#
		// handler can be compared against the delayed DecomposeAction runtime path.
...
public sealed record DecomposeRewardOrderingStep(DecomposeRewardOrderingStepKind Kind, string Description);
