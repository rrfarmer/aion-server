		...
		return new FindGroupDirectPacketBoundaryTraceReadinessReport(
			...
			[
				...
				new FindGroupDirectPacketBoundaryTraceEvidence(
					...
					"C# focused tests can record disabled CM_FIND_GROUP action 0 acceptance before an opt-in registry direct send of SmFindGroup to the same player.",
					FindGroupDirectPacketBoundaryTraceEvidenceStatus.EvidenceAvailable),
				new FindGroupDirectPacketBoundaryTraceEvidence(
			...
			[
				...
				"Before enabling live CmFindGroup direct actions, add a ProcessPacketAsync boundary trace or runtime/socket comparison for action 0.",
