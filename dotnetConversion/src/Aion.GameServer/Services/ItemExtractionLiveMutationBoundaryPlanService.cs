...
{
	// Java parity: model/templates/item/actions/ExtractAction.act and services/EnchantService.breakItem
	// define the live extraction boundary where target delete, tool consume, reward add, and final animation ordering diverge from the staged C# plan.
	public const string JavaExtractActionSource = "game-server/src/com/aionemu/gameserver/model/templates/item/actions/ExtractAction.java";

	public const string JavaEnchantServiceSource = "game-server/src/com/aionemu/gameserver/services/EnchantService.java#breakItem";

	public const string CSharpPlannerSource = "dotnetConversion/src/Aion.GameServer/Services/EnchantService.cs#CreateBreakItemPlan";

	public static ItemExtractionLiveMutationBoundaryPlan CreatePlan()
	{
		// Java parity: this plan documents the Java extraction mutation order and the missing runtime
		// boundaries still required for C# to represent partial-success paths and final animation results.
...
public sealed record ItemExtractionLiveMutationBoundaryStep(ItemExtractionLiveMutationBoundaryStepKind Kind, string Description);
