...
public static class PlayerProtectionActiveTaskReportService
{
	public static PlayerProtectionActiveTaskReport CreateReport(PlayerProtectionActiveTaskAdapterResult result) =>
		CreateReport(result.Status, result.Plan, result.FanoutPlan, result.MutatedVisualState, result.IsLive);
	...
	{
		// Java parity: PlayerController.startProtectionActiveTask and stopProtectionActiveTask interleave
		// branch guards, visual mutation, AttackUtil side effects, packet fanout, scheduler work, and the
		// spawned-only stop branch. This report flattens that Java order into auditable rows.
		...
		switch (step)
		{
			case PlayerProtectionActiveTaskPlanStep.CheckProtectionActive:
				Add(
					...
					PlayerProtectionActiveTaskReportRowKind.PlannedMetadata,
					false,
					"Branch is derived from current BLINKING visual state."
				);
				break;
			case PlayerProtectionActiveTaskPlanStep.SetBlinkingVisualState:
				Add(
					...
					PlayerProtectionActiveTaskReportRowKind.LiveStateBoundary,
					mutatedVisualState,
					"Only this visual-state mutation may be executed by the opt-in adapter."
				);
				break;
			case PlayerProtectionActiveTaskPlanStep.CancelCastOnPlayer:
				Add(
					...
					PlayerProtectionActiveTaskReportRowKind.UnsupportedSideEffect,
					false,
					"Live cast cancellation is not executed."
				);
				break;
			case PlayerProtectionActiveTaskPlanStep.RemovePlayerFromTargets:
				Add(
					...
					PlayerProtectionActiveTaskReportRowKind.UnsupportedSideEffect,
					false,
					"Live attacker target cleanup is not executed."
				);
				break;
			...
			case PlayerProtectionActiveTaskPlanStep.ScheduleProtectionActiveTask:
				Add(
					...
					PlayerProtectionActiveTaskReportRowKind.SchedulerIntent,
					false,
					"Live scheduler mutation is disabled."
				);
				break;
			...
			case PlayerProtectionActiveTaskPlanStep.CancelProtectionActiveTask:
				Add(
					...
					PlayerProtectionActiveTaskReportRowKind.SchedulerIntent,
					false,
					plan.ShouldCancelTask
						? "Represented task cancellation is planned only."
						: "Java still calls cancelTask; current C# inputs represent no stored task."
				);
				break;
			...
			case PlayerProtectionActiveTaskPlanStep.NotifyAiOnMove:
				Add(
					...
					PlayerProtectionActiveTaskReportRowKind.UnsupportedSideEffect,
					false,
					"Live AI movement notification is not executed."
				);
				break;
			default:
	...
	private static void AddBroadcast(ICollection<PlayerProtectionActiveTaskReportRow> rows, PlayerProtectionActiveTaskFanoutPlan fanoutPlan)
	{
		Add(
			rows,
			"SM_PLAYER_STATE",
	...
	{
		rows.Add(new PlayerProtectionActiveTaskReportRow(rows.Count + 1, javaArtifact, javaOperation, kind, isLive, notes));
	}
}
