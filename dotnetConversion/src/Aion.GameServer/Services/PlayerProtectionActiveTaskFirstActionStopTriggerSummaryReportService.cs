...
public static class PlayerProtectionActiveTaskFirstActionStopTriggerSummaryReportService
{
	...
	{
		// Java parity: first-action protection stops are spread across CM_MOVE, CM_MOVE_IN_AIR, attack,
		// cast, item, dialog, composition, and emotion handlers. This summary condenses the packet-level
		// audit into wiring categories and remaining runtime blockers.
		...
		var hasBlocker = rowArray.Any(row =>
			row.Status
				is PlayerProtectionActiveTaskFirstActionStopTriggerSummaryStatus.BlockedProductionWiring
					or PlayerProtectionActiveTaskFirstActionStopTriggerSummaryStatus.NeedsRuntimeVerification
		);

		return new PlayerProtectionActiveTaskFirstActionStopTriggerSummaryReport(
			rowArray,
			HasAllKnownPacketSources: hasAllSources,
			HasThresholdedMovementSource: rowArray.Any(row =>
				row.Category == PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.ThresholdedMovement
			),
			HasEarlyActionStopSources: rowArray.Any(row =>
				row.Category
					is PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.EarlyActionStop
						or PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.EarlyAfterNullGuardActionStop
			),
			HasLateGuardedEmotionSource: rowArray.Any(row =>
				row.Category == PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.LateGuardedEmotionStop
			),
			HasProductionWiringBlocker: rowArray.Any(row =>
	...
	{
		rows.Add(
			new PlayerProtectionActiveTaskFirstActionStopTriggerSummaryRow(
				rows.Count + 1,
				auditRow.Source,
	...
	private static PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory CategoryFor(
		...
		{
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmMove =>
				PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.ThresholdedMovement,
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmMoveInAir =>
				PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.AcceptedAirMovement,
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmCompositeStones =>
				PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.EarlyAfterNullGuardActionStop,
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmEmotion =>
				PlayerProtectionActiveTaskFirstActionStopTriggerSummaryCategory.LateGuardedEmotionStop,
	...
	private static string NotesFor(PlayerProtectionActiveTaskFirstActionStopTriggerSource source, string auditNotes) =>
		...
		{
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmMove =>
				$"Thresholded movement source; keep exact x/y float inequality and asymmetric z-drop threshold. {auditNotes}",
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmMoveInAir =>
				$"Accepted air-movement source; stop is unconditional after spawned/flying guards. {auditNotes}",
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmCompositeStones =>
				$"Early action source after null-player guard; invalid later composition branches may still have stopped protection. {auditNotes}",
			PlayerProtectionActiveTaskFirstActionStopTriggerSource.CmEmotion =>
				$"Late guarded emotion source; many early returns do not stop protection and side effects may occur before stop. {auditNotes}",
