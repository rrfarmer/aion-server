...
/// <summary>
/// Java parity: instance/pvp/EngulfedOphidanBridgeInstance (Tibald) : BasicPvpInstance. 1:1.
/// </summary>
[InstanceID(301210000)]
public class EngulfedOphidanBridgeInstance : BasicPvpInstance
{
    private const int MAX_PLAYERS_PER_FACTION = 6;
    private int timeInMin = 0;

    public EngulfedOphidanBridgeInstance(WorldMapInstance instance) : base(instance)
    {
    }
    ...
    private void UpdatePointsByGenerators(List<Npc> generators, Race race, bool check)
    {
        if (check && !instanceScore.IsStartProgress())
            return;
        if (generators.Count == 0)
            return;
        int points = generators.Count * 100;
        SendMsg(SM_SYSTEM_MESSAGE.STR_MSG_GET_SCORE(generators[0].GetObjectTemplate().GetL10n(), points));
    ...
    public override void PortToStartPosition(Player player)
    {
        ...
        {
            Aion.GameServer.Services.Teleport.TeleportService.TeleportTo(player, instance, 760.9035f, 575.408f, 578.4075f, (byte)86);
        }
        else
        {
            Aion.GameServer.Services.Teleport.TeleportService.TeleportTo(player, instance, 305.4604f, 488.341f, 598.5838f);
        }
    }
}
