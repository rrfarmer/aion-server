...
/// <summary>Java parity: instance/OccupiedRentusBaseInstance (Tibald, Estrayl) : GeneralInstanceHandler. @InstanceID(300620000). AtomicBoolean→int+Interlocked. onDie boss webs + Vasharti end event walkers; onAggro doors; handleUseItemFinish teleports+applyEffectDirectly+chest skills; onCreatureDetected Xasta event; onSpecialEvent; onEnterInstance race-known elyos swap; isBoss override. 1:1.</summary>
[InstanceID(300620000)]
public class OccupiedRentusBaseInstance : GeneralInstanceHandler
{
    private int isRaceKnown;
    private int isXastaEventStarted;

    public OccupiedRentusBaseInstance(WorldMapInstance instance) : base(instance)
    {
    }
    ...
    public override void HandleUseItemFinish(Player player, Npc npc)
    {
        switch (npc.GetNpcId())
        {
            ...
            case 702688:
                ...
                break;
            case 701151:
                Aion.GameServer.SkillEngine.SkillEngine.GetInstance().GetSkill(npc, 19909, 60, npc).UseNoAnimationSkill();
                npc.GetController().Delete();
                break;
            case 701152:
                Aion.GameServer.SkillEngine.SkillEngine.GetInstance().GetSkill(npc, 19910, 60, npc).UseNoAnimationSkill();
                npc.GetController().Delete();
                break;
            case 701097:
                npc.GetController().Delete();
