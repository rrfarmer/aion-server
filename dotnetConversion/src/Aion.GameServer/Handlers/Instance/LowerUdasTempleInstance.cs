...
/// <summary>Java parity: instance/LowerUdasTempleInstance (xTz, Luzien, Majka) : GeneralInstanceHandler. @InstanceID(300160000); WorldPosition[] trap_positions; AtomicBoolean wasSpawned compareAndSet→int+Interlocked; onInstanceCreate rnd named chests; onEnterInstance spawns traps; handleUseItemFinish deletes traps. 1:1.</summary>
[InstanceID(300160000)]
public class LowerUdasTempleInstance : GeneralInstanceHandler
{
    private static readonly WorldPosition[] trap_positions = {
        new WorldPosition(300160000, 749.4821f, 880.4201f, 154.0f, (byte)0),
    ...
    private readonly List<Npc> traps = new List<Npc>();
    private int wasSpawned;

    public LowerUdasTempleInstance(WorldMapInstance instance) : base(instance)
    {
    }
    ...
    {
        if (Interlocked.CompareExchange(ref wasSpawned, 1, 0) == 0)
        {
            traps.Add((Npc)Spawn(216531, 744.7521f, 885.8238f, 152.7852f, (byte)30)); // Zhanim The Librarian
            foreach (WorldPosition position in trap_positions)
