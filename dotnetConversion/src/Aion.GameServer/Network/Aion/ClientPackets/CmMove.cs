using Aion.Commons.Network;
using Aion.GameServer.Controllers.Movement;
using MovementGlideFlag = global::Aion.GameServer.Controllers.Movement.GlideFlag;

namespace Aion.GameServer.Network.Aion.ClientPackets;

public sealed class CmMove : GameClientPacket
{
	public CmMove(int opCode, IReadOnlySet<GameConnectionState> validStates)
		: base(opCode, validStates)
	{
	}

	public float X { get; private set; }

	public float Y { get; private set; }

	public float Z { get; private set; }

	public byte Heading { get; private set; }

	public byte Type { get; private set; }

	public float TargetX { get; private set; }

	public float TargetY { get; private set; }

	public float TargetZ { get; private set; }

	public float VectorX { get; private set; }

	public float VectorY { get; private set; }

	public float VectorZ { get; private set; }

	public byte GlideFlag { get; private set; }

	public int GeyserLocationId { get; private set; }

	public int VehicleUnk1 { get; private set; }

	public int VehicleUnk2 { get; private set; }

	public float VehicleX { get; private set; }

	public float VehicleY { get; private set; }

	public float VehicleZ { get; private set; }

	public bool HasManualPosition => MovementMask.HasManualPosition(Type);

	public bool IsAbsolute => MovementMask.Has(Type, MovementMask.ABSOLUTE);

	public bool IsGliding => MovementMask.Has(Type, MovementMask.GLIDE);

	public bool IsVehicle => MovementMask.Has(Type, MovementMask.VEHICLE);

	protected override void ReadPayload(PacketBuffer buffer)
	{
		// Java parity: network/aion/clientpackets/CM_MOVE.readImpl.
		X = buffer.ReadF();
		Y = buffer.ReadF();
		Z = buffer.ReadF();
		Heading = buffer.ReadC();
		Type = buffer.ReadC();

		if (HasManualPosition)
		{
			if (IsAbsolute)
			{
				TargetX = buffer.ReadF();
				TargetY = buffer.ReadF();
				TargetZ = buffer.ReadF();
			}
			else
			{
				VectorX = buffer.ReadF();
				VectorY = buffer.ReadF();
				VectorZ = buffer.ReadF();
				TargetX = VectorX + X;
				TargetY = VectorY + Y;
				TargetZ = VectorZ + Z;
			}
		}

		if (IsGliding)
		{
			GlideFlag = buffer.ReadC();
			if (GlideFlag == MovementGlideFlag.Geyser)
				GeyserLocationId = buffer.ReadC();
		}

		if (IsVehicle)
		{
			VehicleUnk1 = buffer.ReadD();
			VehicleUnk2 = buffer.ReadD();
			VehicleX = buffer.ReadF();
			VehicleY = buffer.ReadF();
			VehicleZ = buffer.ReadF();
		}
	}
}
