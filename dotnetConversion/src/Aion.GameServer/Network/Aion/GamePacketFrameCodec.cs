...
public static class GamePacketFrameCodec
{
	...
	public static PacketBuffer CreatePayloadBuffer(byte[] frame, bool strictReads = true)
	{
		if (frame.Length < 3)
			throw new ArgumentException("Frame must include a two-byte length and packet payload.", nameof(frame));
