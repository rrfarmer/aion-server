...
public sealed record PlayerPetRepositoryCommandPlan(
	string JavaArtifact,
	string Sql,
	IReadOnlyList<PlayerPetRepositoryParameter> Parameters);
...
{
	public const string SaveFeedStatusSql =
		"UPDATE player_pets SET hungry_level = ?, feed_progress = ?, reuse_time = ? WHERE id = ?";

	public const string SaveDopingBagSql =
		"UPDATE player_pets SET dopings = ? WHERE id = ?";

	public const string SetTimeSql =
		"UPDATE player_pets SET reuse_time = ? WHERE id = ?";
	...
	public const string RemovePlayerPetSql =
		"DELETE FROM player_pets WHERE id = ?";

	public const string LoadPlayerPetsSql =
		"SELECT * FROM player_pets WHERE player_id = ?";

	public const string UpdatePetNameSql =
		"UPDATE player_pets SET name = ? WHERE id = ?";
	...
	public const string LoadUsedIdsSql =
		"SELECT id FROM player_pets";

	public static PlayerPetRepositoryCommandPlan SaveFeedStatus(
		int petObjectId,
		int hungryLevel,
		int feedProgress,
		long reuseTime)
	{
		return new PlayerPetRepositoryCommandPlan(
			...
			SaveFeedStatusSql,
			[
				new("hungry_level", hungryLevel),
				new("feed_progress", feedProgress),
				new("reuse_time", reuseTime),
				new("id", petObjectId),
			]);
	}
	...
	public static PlayerPetRepositoryCommandPlan SaveDopingBag(int petObjectId, IReadOnlyList<int> itemIds)
	{
		return new PlayerPetRepositoryCommandPlan(
			...
			SaveDopingBagSql,
			[
				new("dopings", string.Join(",", itemIds)),
				new("id", petObjectId),
			]);
	}
	...
	{
		return new PlayerPetRepositoryCommandPlan(
			"com.aionemu.gameserver.dao.PlayerPetsDAO.getUsedIDs",
			LoadUsedIdsSql,
			[]);
	}
}
